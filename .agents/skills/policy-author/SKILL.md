...
description: |-
  ...
  • enforce a rules file — make a AGENTS.md / AGENTS.md real instead of advisory;
...
AGENTS.md."* Prose rules are advisory — the agent reads them (maybe) and forgets them under
...
`decision.behavior`; `instruct` uses `additionalContext`; the non-Codex CLIs use flat
...
## 3. Enforcing a rules file (AGENTS.md / AGENTS.md / system prompts)
...
```js
// derived-from: AGENTS.md § "Workflow rules" — "Never push a branch that is
...
| `references/rules-files.md` | Enforcing a AGENTS.md / AGENTS.md (§3) — classification and worked examples |
