...
### Features
- Add `Setup` event to `HOOK_EVENT_TYPES` — fires when Claude Code starts with `--init-only`, or `--init`/`--maintenance` in `-p` mode; synced from upstream Claude Code docs (#282)
