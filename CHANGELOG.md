...
### Fixes
- `scripts/translate-docs`: switch the Tier 2/3 default from the dated snapshot ID `claude-haiku-4-5-20251001` to the alias `claude-haiku-4-5`. The Translate Docs CI workflow's API key is scoped to model aliases (`claude-sonnet-4-6`, `claude-haiku-4-5`) so the dated form was rejected with `key_model_access_denied` on every Tier 2/3 run (#300).
