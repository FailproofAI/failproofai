...
describe("Antigravity tool-name canonicalization", () => {
  it("canonicalizes agy tool ids to Claude builtins", () => {
    ...
    expect(canonicalizeToolName("write_file", "antigravity")).toBe("Write");
    expect(canonicalizeToolName("list_directory", "antigravity")).toBe("LS");
    expect(canonicalizeToolName("grep_search", "antigravity")).toBe("Grep");
    expect(canonicalizeToolName("find_filepath", "antigravity")).toBe("Glob");
...
describe("Antigravity tool-input canonicalization", () => {
  ...
  it("leaves non-Bash tool input untouched (no map entry)", () => {
    const input = { file_path: "/x", content: "y" };
    expect(canonicalizeToolInput("Write", input, "antigravity")).toBe(input);
  });
});
