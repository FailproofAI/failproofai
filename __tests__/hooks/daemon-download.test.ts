...
/**
 * The daemon binary reaches users through exactly one channel now — the
 * GitHub Release for this CLI's own version — so this file covers that
 * channel end to end against a real local HTTP server rather than a mocked
 * `fetch`: URL construction, checksum verification, decompression, and the
 * atomic install. What it is guarding is an executable that a service manager
 * will run at login, so every rejection path asserts that nothing was left on
 * disk.
 */
import { describe, it, expect, beforeEach, afterEach, vi } from "vitest";
import { createServer, type Server } from "node:http";
import { existsSync, mkdtempSync, readFileSync, rmSync, statSync, readdirSync, mkdirSync } from "node:fs";
