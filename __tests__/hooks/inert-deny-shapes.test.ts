...
describe("deny shapes the CLI actually reads", () => {
  ...
  // Hermes emits the same block shape on every installed event; only
  // pre_tool_call is read by upstream. We do not install pre_verify, so there
  // is no Stop branch to special-case.
  it("hermes events carry the plain blocked message", async () => {
