...
describe("policy catalog / implementation split", () => {
  describe("the join", () => {
    it("keeps catalog and joined view the same length and order", () => {
      expect(POLICY_CATALOG).toHaveLength(43);
    ...
    it("assigns 43 DISTINCT implementations, never a shared wrapper", () => {
      ...
      // no symptom anywhere.
      expect(new Set(BUILTIN_POLICIES.map((p) => p.fn.toString())).size).toBe(43);
    });
    ...
    it("has unique names", () => {
      ...
      // from the other copy.
      expect(new Set(BUILTIN_POLICIES.map((p) => p.name)).size).toBe(43);
    });
