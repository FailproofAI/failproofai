...
const EXPECTED_ORDER = [
  ...
  "block-disk-destruction", "block-gh-destructive", "block-mass-kill", "block-no-verify",
  "block-indirect-exec", "block-chmod-777",
];

describe("policy catalog / implementation split", () => {
  describe("the join", () => {
    it("keeps catalog and joined view the same length and order", () => {
      expect(POLICY_CATALOG).toHaveLength(45);
      expect(BUILTIN_POLICIES).toHaveLength(45);
    ...
    it("assigns 45 DISTINCT implementations, never a shared wrapper", () => {
      ...
      // no symptom anywhere.
      expect(new Set(BUILTIN_POLICIES.map((p) => p.fn.toString())).size).toBe(45);
    });
    ...
    it("has unique names", () => {
      ...
      // from the other copy.
      expect(new Set(BUILTIN_POLICIES.map((p) => p.name)).size).toBe(45);
    });
