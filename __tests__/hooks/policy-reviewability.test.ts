 ...
 * with the remedy, this build's builtins must report the seven Jev may clear,
...
/** The seven in this build; pinned by `policy-authority-table.test.ts` too. */
...
describe("counting what Jev may clear", () => {
  ...
  it("reports the seven reviewable builtins, and diagnoses nothing", () => {
    const coverage = { ...countReviewable(POLICY_CATALOG), customFiles: 0 };
    expect(coverage.reviewable).toBe(7);
    ...
    expect(reviewableSummary(coverage)).toBe(
      `7 of ${POLICY_CATALOG.length} enabled policies are reviewable: ` +
...
describe("surveying a real machine", () => {
  ...
  it("a pack from before this release: zero of N, with the remedy", () => {
    ...
    // here would report seven clears that cannot happen.
  ...
  it("a pack built by this release: the seven it marks, and no complaint", () => {
  ...
  it("falls back to this build's builtins while no pack is installed", () => {
    ...
    const coverage = surveyReviewableCoverage(project);
    expect(coverage).toEqual({ enabled: 9, reviewable: 7, customFiles: 0 });
    expect(reviewableProblem(coverage)).toBeNull();
  });
