...
describe("the final scrub pass", () => {
  ...
  it("builds the whole envelope inside the hook's budget", () => {
    ...
    // The guard on the workload, not the assertion: it says the pass above
    // actually ran over a full envelope. It was 10 000 when an envelope carried
    // as much as the caller sent; T3's budget bounds what is BUILT at
    // MAX_AGENT_REQUEST_CHARS + MAX_CONTEXT_CHARS, so 634 KB of tool input is
    // now read down to ~160 KB and ~3 300 redactions is the ceiling, not a
    // shortfall. The timing assertion below is unchanged.
    expect(redactions).toBeGreaterThan(3_000);
