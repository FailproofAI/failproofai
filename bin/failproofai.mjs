...
if (hookIdx >= 0) {
  if (!args[hookIdx + 1]) {
    ...
    console.error("Usage: failproofai --hook <event> [--cli <claude|codex|copilot|cursor|opencode|pi|gemini|hermes|openclaw>]");
    process.exit(1);
  }
...
COMMANDS
  ...
  policies --install, -i         Enable policies in agent CLI settings
    ...
    --cli claude|codex|copilot|cursor|opencode|pi|gemini|hermes|openclaw
                                   Agent CLI(s) to install for; space-separated
                                   (e.g. --cli claude codex copilot cursor opencode pi gemini hermes openclaw) or repeated.
...
EXAMPLES
  ...
  failproofai policies --install --cli claude codex copilot cursor opencode pi gemini hermes openclaw
...
`.trimStart());
    ...
    if (isInstall) {
      ...
      const VALID_CLIS = new Set(["claude", "codex", "copilot", "cursor", "opencode", "pi", "gemini", "hermes", "openclaw"]);
      const cliFlagValues = [];
      ...
      for (const idx of cliFlagIdxs) {
        ...
        if (consumed === 0) {
          throw new CliError("Missing value(s) for --cli. Usage: --cli claude codex copilot cursor opencode pi gemini hermes openclaw (or any subset)");
        }
      }
