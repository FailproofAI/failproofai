...
- policies are evaluated locally without waiting for the cloud;
...
The local daemon reconciles these changes automatically. A user does not run a sync command after a cloud change.
...
Hook decisions do not depend on a network request. The daemon loads the last verified policy generation before accepting events and continues enforcing it while the cloud is unavailable.
...
- Cloud outage does not prevent local decisions and is visible as freshness degradation.
