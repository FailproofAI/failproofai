...
In managed and system scope the hook client, service definition, protected policy store, pinned policy runtime, and active schema catalog are owned by the service account or root and are not writable by enrolled users. Every component of the path to each of them is owned the same way, including the socket's parent directory — otherwise an enrolled user could unlink the socket and bind an impostor endpoint that answers `allow`. The daemon accepts evaluation requests from enrolled users but rejects administrative operations unless the peer is the service account, root, or holds an OS-backed administrator authorization. Authentication is based on peer credentials, not a bearer token exposed to the agent environment.
