...
- The observability delivery key is the only secret Phase 1 handles. It lives in the user's own configuration alongside the rest of the collector's settings, readable by that user, never in a service definition or process argument, and erased unconditionally on uninstall.
