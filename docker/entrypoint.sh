...
#   6. Capture rc; optional webhook telemetry.
#
...
GH_TOKEN="${GH_TOKEN:-}"
HOOK_SYNC_WEBHOOK_URL="${HOOK_SYNC_WEBHOOK_URL:-}"
...
RUN_LOG="${RUN_LOG:-/var/log/hook-sync/claude.log}"
...
# Optional telemetry — Slack/Discord-compatible webhook via node (no python needed).
# No-op when HOOK_SYNC_WEBHOOK_URL is empty; never fails the run.
...
# ---------------- 1. env validation ----------------
if [ -z "$CLAUDE_CODE_OAUTH_TOKEN" ]; then log "ERROR: CLAUDE_CODE_OAUTH_TOKEN not set"; notify fail "hook-sync: missing CLAUDE_CODE_OAUTH_TOKEN"; exit 64; fi
if [ -z "$GH_TOKEN" ]; then log "ERROR: GH_TOKEN not set"; notify fail "hook-sync: missing GH_TOKEN"; exit 64; fi
notify start "hook-sync: run started"
...
if [ ! -f "$PROMPT_FILE" ]; then log "ERROR: prompt missing at $PROMPT_FILE"; notify fail "hook-sync: prompt file missing ($PROMPT_PATH)"; exit 65; fi
...
# Capture claude's real exit code without pipefail's `|| true` clobbering PIPESTATUS.
set +e
script -qefc "$RUN_SH" /dev/null | tee "$RUN_LOG"
rc=${PIPESTATUS[0]}
set -e
rm -f "$RUN_SH"
...
# ---------------- 6. telemetry ----------------
tail_body="$(tail -c 1500 "$RUN_LOG" 2>/dev/null || true)"
if [ "$rc" -eq 0 ]; then
  notify ok "hook-sync: run OK (branch $BRANCH)"$'\n'"$tail_body"
else
  notify fail "hook-sync: run FAILED rc=$rc (branch $BRANCH)"$'\n'"$tail_body"
fi
exit "$rc"
