...
Failproof AI separates policy selection from machine and collector settings. This keeps repository policy choices reviewable while credentials and daemon state stay outside the repository.
...
<Tabs>
  <Tab title="Dashboard">
    ...
    <Info>Screenshot placeholder: local policy dashboard with scope selector and enabled policy list.</Info>
  </Tab>
  <Tab title="CLI">
    ```bash
...
<Tabs>
  <Tab title="Dashboard">
    ...
    <Info>Screenshot placeholder: local policy parameter editor and the resulting Cloud decision.</Info>
  </Tab>
  <Tab title="CLI">
...
| Path | Purpose |
| --- | --- |
| `config.json` | Non-secret daemon, collector, audit, and telemetry settings |
