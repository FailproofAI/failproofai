...
<Tabs>
  <Tab title="Dashboard">
    ...
    2. Copy the secret and connect the machine with `failproofai config`.
    3. Open **Administration → Deploy policies** and confirm the machine appears with its expected label.
    4. Deploy a policy version to that machine.
    5. Open **Observe → Policy** after a test session and confirm the machine reports its decisions.

    <Info>Screenshot placeholder: collector and policies key presets, the enrolled machine in Deploy policies, and its policy decisions.</Info>
  </Tab>
  <Tab title="CLI">
    ```bash
...
`failproofai audit --scheduled` runs the local audit headlessly. Exit code `75` means another audit already holds the single-run lock.
...
<Tabs>
  <Tab title="Dashboard">
    For a policy rollout problem, first open **Administration → Deploy policies** and roll the affected machines back to a known version. Use the local pause only when the deployment cannot be corrected quickly. After recovery, confirm the machine's version and health in the deployment view.

    <Info>Screenshot placeholder: machine deployment detail showing current version, prior version, and health.</Info>
  </Tab>
  <Tab title="CLI">
    ```bash
