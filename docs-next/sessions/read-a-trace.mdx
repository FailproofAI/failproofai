...
<Tabs>
  <Tab title="Dashboard">
    ...
    4. Select an event to open the detail panel. Use **export** to download evaluator JSON.
    5. Copy the page URL after selecting an event to share a deep link to that evidence.
...
<Steps>
  <Step title="Check the session header">
    Confirm the agent, environment, start and end time, outcome, and overall duration.
  </Step>
  <Step title="Scan the event spine">
    Look for errors, unusually long spans, repeated tool names, human waits, and denied policy decisions.
  </Step>
