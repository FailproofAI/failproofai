...
<Tabs>
  <Tab title="Dashboard">
    ...
    3. After running the CLI connection command, go to **Administration → Deploy policies** and confirm the machine appears.
    4. Go to **Observe → Events** and confirm its first event arrives.

    <Info>Screenshot placeholder: Key creation, new machine in enforcement, and first live event.</Info>
  </Tab>
  <Tab title="CLI">
    ```bash
