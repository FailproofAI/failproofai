...
2. **The SDK.** `pip install failproofai-sdk` (or `uv add failproofai-sdk`). The skill knows the install path, including the one command to avoid — `pip install agenteye` resolves to a stranded release of an older CLI, not to this SDK.
