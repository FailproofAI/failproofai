...
After each hook event, the handler appends a JSONL line to `~/.failproofai/hook-activity/current.jsonl`, which rotates into `page-<timestamp>-<seq>.jsonl` once it reaches a page:
