...
**`customPoliciesPaths` / `customPoliciesPath`** — первый уровень, определивший любую форму, побеждает.
...
**Объединённая загрузка:** сканируются обе директории конвенциональных политик — проектная и пользовательская. Все совпадающие файлы с обоих уровней загружаются (в отличие от `customPoliciesPath`, которая использует принцип «первый уровень побеждает»).
