...
**`customPoliciesPaths` / `customPoliciesPath`** - phạm vi đầu tiên xác định bất kỳ dạng nào sẽ thắng.
...
Ngoài `customPoliciesPath` rõ ràng, failproofai sẽ tự động khám phá và tải các tệp chính sách từ các thư mục `.failproofai/policies/`:
...
**Tải kết hợp:** Cả thư mục quy ước dự án và người dùng đều được quét. Tất cả các tệp khớp từ cả hai mức đều được tải (không giống `customPoliciesPath` sử dụng phạm vi thắng đầu tiên).
