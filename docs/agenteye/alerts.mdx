...
Alerts evaluate a rule on a schedule and notify you when something crosses a threshold. They turn AgentEye from a passive dashboard into a paging surface for the things that matter: error rate spikes, latency regressions, evaluator score drops, or any custom event you can express as a ClickHouse query.

This guide covers what an alert is, the five trigger types, the four notification channels, the incident lifecycle, and the operational knobs.
...
Reads `agenteye.evaluations` and compares the average of a named score across a window.
...
Every alert has two integer knobs in addition to the trigger:
...
Each alert can attach any combination of these four. Per-channel credentials (Slack webhook URL, generic webhook URL + signing secret, default email recipients) are configured once in **`/settings`** and referenced by key from each alert. This way one Slack channel can serve many alerts without each one storing its own copy of the webhook URL.
...
- `incidents:write`: open manual (ad-hoc) incidents that are not tied to an alert.
- `incidents:ack`: acknowledge, assign, comment on, and resolve incidents.

> **Legacy `alerts:ack`.** Keys and operators that were granted the older `alerts:ack` token keep working: it is honored as `incidents:ack` (and implies `incidents:read`), so existing on-callers retain their access without re-issuing credentials. Issue new grants with the `incidents:*` family.

Grant on API keys (`POST /keys`) and operators (`PUT /users/:id`). The dashboard's `PermGate` locks the relevant buttons when a permission is missing; you'll see `// 403` next to the action.

> **Email-recipient picker.** The alert editor's recipient picker lists your org's members so you can pick them by name. It loads for any operator holding `alerts:read` or `alerts:write`; viewing your team's directory for this purpose does **not** require `users:read`, and the picker returns only member email addresses, never full user records.
...
Before relying on a new alert, verify the full path end-to-end:

1. **Save the alert** with `enabled = true` and at least one channel.
2. **Press the *test* button** on the alert detail page. The server fans a synthetic firing notification through every configured channel and records each delivery in `alert_notifications` with `event = test`. Check the destination (Slack room, mailbox, webhook log).
3. **Induce a real breach** (e.g. write `error` events into ClickHouse for a `metric_threshold` alert) and wait one `eval_interval_secs`. Confirm:
   - the incident appears on `/incidents`,
   - the firing notification arrives,
   - the *value* in the notification matches what the dashboard's `/queries/run` returns for the same window.
4. **Resolve the incident** — press *resolve* on `/incidents/:id`. Confirm it transitions to `resolved`. (Automatic resolution when the condition clears is planned but not currently enabled — an open incident stays open until an operator resolves it.)
5. **Acknowledge** an open incident from `/incidents/:id` and confirm the action appears on the *activity* timeline attributed to your operator account.
...
|---|---|
| Alert never fires | `enabled = false`, or no channels attached, or the underlying CH query returns 0 rows. Use *test* to confirm channels; use `/queries/run` to confirm the metric. |
