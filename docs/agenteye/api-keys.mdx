---
title: "API Keys"
description: "AgentEye API Keys documentation."
---
...
| Permission | HTTP routes | What it allows |
|---|---|---|
| `settings:read` | `GET /settings`, `GET /settings/schema` | View dashboard-managed operational settings (allowed sign-ins, session and OTP TTLs) plus the metadata that describes each setting. |
| `settings:write` | `PUT /settings/:key` | Edit operational settings. Changes take effect on the next request without restarting the server. |

![The Settings page: dashboard-managed operational settings such as allowed sign-ins and session/OTP lifetimes, editable without a restart](/agenteye/images/settings.png)
...
![The API Keys page: a card per key showing its name, granted permissions, and creation time, with regenerate and disable actions; protected keys like `admin` are marked](/agenteye/images/api-keys.png)
