...
Each `model_response` event shows a **context-fill pill** — how full the model's context window got that turn (`healthy` → `watch` → `compacting` → `reset context`). It's computed automatically at ingest against each model's window size; no configuration is required. On the dashboard's **Settings → model context windows** page, an org edits per-model window sizes (0–1,000,000 tokens; a size of `0` means "unknown", so no pill is shown). Every model an org sends is listed there automatically.
...
- **Image tags**: Pin to a specific version tag (e.g. `server:v0.0.1-beta.41`) in production rather than a floating tag to avoid unintended upgrades. Current beta builds publish under `beta-latest`; `latest` is assigned only to stable releases.
...
| `v<version>` | Pinned version, e.g. `v0.0.1-beta.41` (recommended for production) |
