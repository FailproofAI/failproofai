...
Use the `server` image tag pinned in your overlay's `kustomization.yaml` (the acme overlay pins `v0.0.1-beta.41`; the base `server/deployment.yaml` ships `v0.0.1-beta.43`). Substitute the tag below for the one you are deploying so this check does not drift across releases:

```bash
kubectl run test-pull \
  --image=ghcr.io/agenteye-enterprise/server:v0.0.1-beta.41 \
