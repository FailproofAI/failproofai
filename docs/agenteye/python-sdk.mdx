...
Upgrading an existing integration? Uninstall `agenteye`, install `failproofai-sdk`, and change `import agenteye` to `import failproofai_sdk`. Nothing else moves: every method, argument, and emitted field is identical, and so is everything on disk — the spool is still `~/.agenteye/` and `AGENTEYE_HOME` still overrides it.
