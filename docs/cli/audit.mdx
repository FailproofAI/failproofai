...
- **Whole-result cache** at `~/.failproofai/audit-dashboard.json` (mode 0600). Lets the dashboard render instantly on navigation without re-running. Also rejected on read past the **7-day TTL** — `/audit` then falls through to the empty state with an `audit expired — re-audit to refresh` banner. Click `[ re-audit ]` at the top of the page (or `[ re-audit now ]` near the bottom) to **clear the cache and run a fresh scan** — it sends `noCache: true`, so re-audit re-scans every transcript instead of returning the cached result. Progress streams inline on the bar's freshness gauge (a fixed strip echoes it once the bar scrolls out of view) and the result swaps in place on success (no page reload); a failed re-audit keeps the previous report.
