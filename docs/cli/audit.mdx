...
- **Per-transcript cache** at `~/.failproofai/cache/audit/<sha1>.json` keyed by `(mtime, size, engineVersion, detectorVersion, cachedAt)`. Invalidates automatically when policy or detector code changes, and is rejected on read once it's older than **7 days** so long-lived results don't outlive evolving detector intent.
