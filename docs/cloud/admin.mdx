---
title: "Admin"
description: "Keys, users, usage, and settings — scoped so a machine can report activity without ever gaining read or admin powers."
icon: key
---

This page covers the whole **admin** section of the dashboard — [Keys](#the-three-keys-most-teams-need),
[Users](#users), [Usage](#usage), and [Settings](#settings). Most of it is about keys,
...
<Note>
  **`policies:pull`** is the permission a machine needs to receive [managed
  policies](/cloud/policy-editor). It is verified independently of `events:add` at
...
**admin → Users** is where membership is managed. Sign-in is passwordless: inviting someone
mails them a six-digit one-time passcode, and there is no password to store or rotate. Each
member's granted scopes show as chips on their card, and a permission change takes effect on
their **next request** — no relogin — with an email to the affected person.

Disabling a user revokes their sessions immediately; re-enabling restores them. The
permissions behind the page are [`users:create` / `users:read` / `users:update` /
`users:delete`](#dashboard-users).

---

## Usage

**admin → Usage** reports what this organization consumed in its **current fixed 30-day
metering window**: telemetry ingested, evaluations, workspace objects, audits, API keys, and
members. It is read-only — it reports consumption, and does not apply or display limits.

Needs `usage:read`. From the terminal:
...
The JSON form returns the dashboard's response unchanged: `org_id`, `billing_anchor`,
...
**admin → Settings** holds per-organization configuration. Two are worth setting on day one:

| Setting | Why it matters early |
|---|---|
| **Default alert recipients** (`alerts.email_default_recipients`) | A *failed* audit run mails this list even when the audit has no email channel of its own. An audit that has quietly stopped producing findings is exactly the thing nobody notices, so make sure this is a list someone reads. |
| **Model context windows** | Corrects or adds the context-window size for a model FailproofAI does not recognize, so the context-fill readouts on [Performance](/cloud/performance) are right for private or newly released models. |

Reading settings needs `settings:read`; changing one needs `settings:write`. From the
terminal:

```bash
agenteye settings list                                    # key · value · type · updated
agenteye settings schema                                  # what each key is and what it accepts
agenteye settings set session_ttl_secs --value 86400
agenteye settings set alerts.email_default_recipients \
  --json-value '["oncall@example.com"]'
```

`settings set` takes the value exactly one way — `--value` for a scalar, `--json-value` for
an array or object, or `--file`/stdin for JSON — checks the key exists, shows the
before → after, and confirms before writing. An unknown key exits 6.
