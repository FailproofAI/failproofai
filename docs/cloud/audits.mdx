---
title: "Audits"
description: "A recurring investigation across every session in your fleet, handing you a ranked, evidence-backed list of what to fix."
---
...
- **Drift versus a baseline**: behavior moving away from a known-good window.
...
Credential findings link to matched events without copying the secret into the finding. If
retention removed an event, the page reports that it is unavailable.

The server discards recommendations whose cited sessions do not resolve. Remaining findings
are ranked by impact.

## Turn a fix into a policy

Create a prefilled recurrence alert from any finding to detect the pattern again.

## Where to find it

Audits live in the dashboard at **`/<org-slug>/audits`** (sidebar → **analyse** → **Audits**). Viewing runs and findings needs **`audits:read`**; creating, editing, and triaging audits needs **`audits:write`**. Set an audit's scope and cadence, then hit **Run now** whenever you want results immediately instead of waiting for the next scheduled pass.

## Related

- [Alerts](/cloud/alerts): get paged the moment a threshold you already know about is crossed.
- [Online evals](/cloud/evaluations): score every run so quality regressions surface on their own.
- [Errors](/cloud/errors): group and follow the errors your agents throw.
- [Issues](/cloud/issues): track an issue an audit turns up through to its fix.
