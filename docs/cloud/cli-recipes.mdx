...
## Triage an issue non-interactively

```bash
id=$(agenteye --json issues list --state firing | jq -r '.issues[0].id')
agenteye issues ack "$id"
agenteye issues assign "$id" --assignee you@corp.com
...
| `alerts list` / `issues list` | `{"alerts": [...]}` / `{"issues": [...]}` |
...
- [API keys](/cloud/admin): create and scope the keys the CLI, SDK, and collector authenticate with.
