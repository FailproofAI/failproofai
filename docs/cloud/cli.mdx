...
Drive all of FailproofAI Cloud from the terminal or a script: no dashboard round-trips. The `agenteye` CLI queries your data (sessions, event logs, evaluations) and administers your org (API keys, users, settings, usage, alerts, audits, issues, saved queries), so reach for it when you want to automate a check, wire FailproofAI Cloud into CI, or let a coding agent inspect production. Every command supports a `--json` flag, so it works equally well for you at a prompt or for a coding agent (Claude Code, Cursor) shelling out and parsing the result.

With one binary you can:

- **Read your data**: `sessions`, `events`, `evals`, `errors` (filter by time, agent, env, score).
- **Find failures**: `audits` (recurring investigations and their findings), `issues` (the triage inbox).
- **Manage your org**: `keys`, `users`, `settings`, `usage`, `alerts`.
...
<Note>
  **The cloud CLI ships under the package name `agenteye`** — the product's former name,
  kept as the distribution name so existing installs keep working. Everything it talks to is
  FailproofAI Cloud.
...
The full surface follows. The CLI has **20 top-level commands**. All read commands accept `--json` and the global options above; run `agenteye <command> -h` (or `<command> <subcommand> -h`) for the exhaustive flag list and JSON shape of any one.
...
### Find failures: `audits` · `issues`

**`audits`**: recurring investigations and the findings they produce, referenced by **name**. `--schedule-anchor` takes an ISO 8601 instant and is **UTC** — there is no timezone support.

```bash
agenteye audits list
agenteye audits show nightly-review
agenteye audits create nightly-review --schedule-interval-secs 86400 \
                                      --schedule-anchor 2026-08-15T02:00:00Z
agenteye audits edit nightly-review --sensitivity high --yes
agenteye audits run nightly-review                   # extra, not a reset — the cadence stays anchored
agenteye audits runs nightly-review
agenteye audits findings nightly-review ; agenteye audits finding <finding-id>
agenteye audits ack <id> ; agenteye audits mute <id> ; agenteye audits dismiss <id>
agenteye audits resolve <id> ; agenteye audits reopen <id> ; agenteye audits assign <id> you@corp.com
agenteye audits context-show nightly-review ; agenteye audits context-set … ; agenteye audits context-refresh …
agenteye audits delete nightly-review --yes
```

**`issues`**: the triage inbox, referenced by id (short ids accepted). `show` prints the full activity log; read it before acting.

```bash
agenteye issues list --state firing                   # also: acknowledged, resolved
agenteye issues count
agenteye issues show <id>
agenteye issues ack <id>
agenteye issues assign <id> you@corp.com              # assignee must be an operator
agenteye issues resolve <id> --yes
agenteye issues open --title "checkout latency regression" --severity critical
agenteye issues comment-add <id> "root cause: upstream 5xx"
agenteye issues comment-list <id> ; agenteye issues comment-delete <id> <comment-id>
agenteye issues subscribe <id> ; agenteye issues unsubscribe <id> ; agenteye issues subscribers <id>
```

`--title` is required when you open a standalone issue and optional with `--alert-id`, where it defaults to the alert's name. The group was called `incidents`; the old spelling and the `incidents:*` permission tokens are still accepted.

### Manage (permission-gated): `keys` · `users` · `settings` · `usage` · `alerts`
...
**`usage`**: what this org consumed in its current fixed 30-day metering window — telemetry, evaluations, workspace objects, audits, keys, and members. Read-only; it reports consumption and does not apply or display limits. Needs `usage:read`.

```bash
agenteye usage
agenteye --json usage | jq '.usage.events_ingested'
```
...
- **[API keys](/cloud/admin)**: the permission model behind `keys create --add …`.
