...
<Note>
  **The SDK ships under the package name `agenteye`** — the product's former name, kept as
  the distribution name so existing installs keep working. Everything it reports to is
  FailproofAI Cloud. The same name on public PyPI is the [cloud CLI](/cloud/cli), a
  different tool: install that one with `pipx` or `uv tool`, never with `pip` into the
  environment your agent runs in.
</Note>
