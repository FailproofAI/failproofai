...
Pass `--cli claude|codex|copilot|cursor|opencode|pi|hermes|factory|devin` to target a specific agent (space-separated or repeated for any subset):
...
failproofai policies --install --cli claude codex copilot cursor opencode pi hermes factory devin
```
