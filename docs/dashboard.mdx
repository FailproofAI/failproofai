...
5. **Findings** — per-finding cards ranked by impact. Each card surfaces what happened, what it costs, an evidence sample with real captured commands, and the failproofai policy that would catch the same pattern (`$ failproofai policy add <slug>`, click-to-copy).
...
Driven by the `failproofai audit` runtime — see [Audit CLI](/cli/audit) for the underlying scan engine, supported flags, and per-transcript cache invariants. The dashboard caches the latest result at `~/.failproofai/audit-dashboard.json` (mode `0600`, single slot, new runs overwrite) so revisits are instant; **both the per-transcript and whole-result caches are rejected on read once they're older than 7 days** so the dashboard never silently serves a week-old result. A top-of-page `[ re-audit ]` bar reads the cache's freshness on a segmented **decay gauge** that drains across the 7-day TTL — green while fresh, amber in the final 24 hours alongside an `expires in 14h` readout, and pink once `audit expired` — next to the last-run timestamp (`audited 3d ago`). Clicking either `[ re-audit ]` (top) or `[ re-audit now ]` (bottom) **clears the cache and runs a fresh scan**: it POSTs `/api/audit/run` with `noCache: true` (so re-audit re-scans every transcript instead of silently returning the identical cached result) and the dashboard polls `/api/audit/status` at 1Hz until the run finishes, then swaps the result in place on success (no full page reload). The bar shows the run inline — an indeterminate gauge sweep + elapsed timer while scanning, and on failure it folds the error inline (pink, with copy keyed off the `RerunError.kind` — `timeout` / `network` / `post_failed` — plus a dismiss). A fixed progress strip echoes that status pinned to the top of the viewport, but only once the bar has scrolled out of view, so there is never a second stacked band; a failed re-audit leaves the prior report intact. Empty state (no cache) and zero-sessions state (cache exists but the scan found no transcripts) are surfaced separately.
