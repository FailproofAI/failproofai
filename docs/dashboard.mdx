...
4. **How to improve** — calm row list, one per prescribed policy: policy name in white, one-line description, install command + copy button on the right side. An `[install all]` button at the section header copies the combined `failproof policy add a b c …` command for every prescribed policy.
