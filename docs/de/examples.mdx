...
Sofort einsatzbereite Beispiele für häufige Szenarien. Jedes Beispiel zeigt, wie die Installation funktioniert und was zu erwarten ist.
...
<Steps>
  ...
  <Step title="Claude Code starten">
    ...
    Richtlinien werden jetzt automatisch bei jedem Tool-Aufruf ausgeführt. Fordern Sie Claude auf, `sudo rm -rf /` auszuführen – der Befehl wird blockiert.
  </Step>
</Steps>
...
Was diese Richtlinien bewirken:
...
## Geheimnisweitergabe verhindern
...
Diese Richtlinien werden bei `PostToolUse` ausgelöst – nachdem ein Tool ausgeführt wurde, bereinigen sie die Ausgabe, bevor der Agent sie zu Gesicht bekommt.
...
Committen Sie eine projektweite Konfiguration, damit jeder Entwickler in Ihrem Team dieselben Richtlinien erhält.
...
Die wirkungsvollste Konfiguration: Committen Sie `.failproofai/policies/` in Ihr Repository mit auf Ihr Projekt zugeschnittenen Richtlinien. Jedes Teammitglied erhält sie automatisch – keine Installationsbefehle, keine Konfigurationsänderungen.
...
| Datei | Inhalt |
|------|---------------|
| `policies-basic.js` | Einsteigerrichtlinien – blockiert Schreibvorgänge in Produktion, Force-Push und weitergeleitete Skripte |
| `policies-notification.js` | Slack-Benachrichtigungen für Leerlauf-Meldungen und Sitzungsende |
| `policies-advanced/index.js` | Transitive Importe, asynchrone Hooks, PostToolUse-Ausgabebereinigung, Stop-Ereignisbehandlung |