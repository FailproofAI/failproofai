...
__tests__/
  hooks/
    ...
    custom-hooks-loader.test.ts   # ESM-Loader, transitive Importe, Fehlerbehandlung
...
E2E-Tests führen das Binary direkt aus dem Repository-Quellcode aus. Erstelle vor dem ersten Durchlauf das CJS-Bundle, das benutzerdefinierte Hook-Dateien verwenden, wenn sie aus `'failproofai'` importieren:
...
Erstelle `dist/` neu, wenn du die öffentliche Hook-API änderst (`src/hooks/custom-hooks-registry.ts`, `src/hooks/policy-helpers.ts` oder `src/hooks/policy-types.ts`).
...
__tests__/e2e/
  helpers/
    hook-runner.ts      # Binary starten, Nutzlast-JSON pipen, Exit-Code + stdout + stderr erfassen
    fixture-env.ts      # Isolierte Temp-Verzeichnisse pro Test mit Konfigurationsdateien
  ...
  hooks/
    ...
    config-scopes.e2e.test.ts      # Konfigurationszusammenführung über Projekt/Lokal/Global
...
// env.cwd    - Temp-Verzeichnis; als payload.cwd übergeben, um .failproofai/policies-config.json zu laden
// env.home   - isoliertes Home-Verzeichnis; kein echtes ~/.failproofai tritt durch
...
Der `forks`-Pool ist wichtig: Thread-basierte Worker teilen sich `globalThis`, was Subprocess-startende Tests stören kann. Prozessbasierte Forks vermeiden dies.

---

## CI

Der vollständige CI-Durchlauf (`bun run lint && bunx tsc --noEmit && bun run test:run && bun run build`) muss erfolgreich sein, bevor ein Merge durchgeführt werden kann. Die E2E-Suite läuft als separater CI-Job parallel dazu.

Siehe [Contributing](../CONTRIBUTING.md) für die vollständige Checkliste vor dem Merge.