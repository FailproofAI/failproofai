...
failproofai verfügt über zwei Test-Suites: **Unit-Tests** (schnell, mit Mocks) und **End-to-End-Tests** (echte Subprocess-Aufrufe).
...
E2E-Tests rufen das echte `failproofai`-Binary als Subprocess auf, leiten einen JSON-Payload über stdin weiter und prüfen die stdout-Ausgabe sowie den Exit-Code. Damit wird der vollständige Integrationspfad getestet, den Claude Code verwendet.

### Setup

E2E-Tests führen das Binary direkt aus dem Repository-Quellcode aus. Vor dem ersten Durchlauf muss das CJS-Bundle gebaut werden, das von benutzerdefinierten Hook-Dateien verwendet wird, wenn sie aus `'failproofai'` importieren:
...
`dist/` muss neu gebaut werden, sobald die öffentliche Hook-API geändert wird (`src/hooks/custom-hooks-registry.ts`, `src/hooks/policy-helpers.ts` oder `src/hooks/policy-types.ts`).

### Struktur der E2E-Tests

```text
__tests__/e2e/
  helpers/
    hook-runner.ts      # Binary starten, Payload-JSON pipen, Exit-Code + stdout + stderr erfassen
  ...
  hooks/
    ...
    custom-hooks.e2e.test.ts       # Custom-Hook-Laden und -Auswertung
...
Der `forks`-Pool ist wichtig: Thread-basierte Worker teilen `globalThis`, was bei Tests, die Subprozesse starten, zu Interferenzen führen kann. Prozessbasierte Forks vermeiden dieses Problem.
...
Weitere Informationen zur vollständigen Pre-Merge-Checkliste finden Sie unter [Contributing](../CONTRIBUTING.md).