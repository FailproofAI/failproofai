...
Unit-Tests befinden sich im Verzeichnis `__tests__/` und verwenden [Vitest](https://vitest.dev) mit `jsdom`.

```text
__tests__/
  hooks/
    builtin-policies.test.ts      # Policy-Logik für jede eingebaute Policy
    hooks-config.test.ts          # Konfigurationsladung und Scope-Zusammenführung
...
E2E-Tests rufen das echte `failproofai`-Binary als Subprocess auf, übergeben eine JSON-Payload über stdin und prüfen die stdout-Ausgabe sowie den Exit-Code. Damit wird der vollständige Integrationspfad getestet, den Claude Code verwendet.

### Setup

E2E-Tests führen das Binary direkt aus dem Repository-Quellcode aus. Vor dem ersten Durchlauf muss das CJS-Bundle gebaut werden, das Custom-Hook-Dateien beim Import von `'failproofai'` verwenden:
...
__tests__/e2e/
  helpers/
    hook-runner.ts      # Binary starten, Payload-JSON einlesen, Exit-Code + stdout + stderr erfassen
...
// env.home   - isoliertes Home-Verzeichnis; kein echtes ~/.failproofai wird eingebunden
...
`createFixtureEnv()` registriert den `afterEach`-Cleanup automatisch.
...
E2E-Tests verwenden `vitest.config.e2e.mts` mit folgenden Einstellungen:
...
Der `forks`-Pool ist wichtig: Thread-basierte Worker teilen sich `globalThis`, was Subprocess-Tests beeinträchtigen kann. Prozessbasierte Forks vermeiden dieses Problem.

---

## CI

Der vollständige CI-Durchlauf (`bun run lint && bunx tsc --noEmit && bun run test:run && bun run build`) muss erfolgreich abgeschlossen werden, bevor ein Merge möglich ist. Die E2E-Suite läuft als separater CI-Job parallel dazu.

Die vollständige Pre-Merge-Checkliste ist unter [Contributing](../CONTRIBUTING.md) zu finden.