...
description: "Patrones de consulta listos para copiar y pegar, y recetas de jq que convierten datos de sesiones, eventos y evaluaciones en algo que un script o agente de código puede automatizar."
---


Extrae datos de sesiones, eventos y evaluaciones (y dispara re-evaluaciones) directamente desde un script o agente de código, con JSON limpio en stdout que se canaliza directamente a `jq`. Estas recetas convierten los datos de Failproof AI Observability en algo que un usuario de terminal o un agente de código con IA (Claude Code, Cursor) puede consultar y automatizar, sin necesidad de navegar por el dashboard.

Los patrones a continuación están listos para copiar y pegar en la CLI de Failproof AI Observability (`agenteye`). Para instalación, autenticación y la lista completa de opciones, consulta [CLI](/es/agenteye/cli); ejecuta `agenteye -h` o `agenteye <command> -h` para la ayuda integrada.
...
2. **Usa `--json` siempre que vayas a parsear la salida.** Los datos van a **stdout** como JSON; los mensajes de estado y errores van a **stderr**, por lo que stdout se mantiene limpio para canalizar a `jq`.
3. **Ramifica según el código de salida**, no por el texto de stderr: `0` ok · `1` error inesperado · `2` argumentos incorrectos · `3` no se puede alcanzar el dashboard · `4` no autenticado o sesión expirada · `5` permiso faltante · `6` recurso no encontrado.
...
`whoami` nunca genera error cuando la sesión está ausente o expirada; en su lugar reporta `logged_in:false`, por lo que un agente puede verificar el estado de autenticación de forma segura. (Aún puede salir con código no cero si no hay base URL configurada o el dashboard no está disponible.)
...
# evaluaciones con puntuación <= 0.5 en helpfulness, para un agente específico
...
# solo las llamadas a herramientas en una sesión (--full es necesario para obtener el payload completo)
agenteye --json events --full --session-id run-001 --event-type tool_use,tool_result --all \
  | jq '.events[].payload'
```

> **Nota:** Por defecto, `events` lee un feed rápido sin payload. Cada evento lleva un `summary` de una línea calculado por el servidor, además de indicadores como `is_error` y conteos de tokens, pero `payload` se devuelve como `{}`. Para obtener el payload completo, agrega `--full` (o `--fields payload`). El feed completo es más lento a escala, así que mantenlo acotado: combina `--full` con un único `--session-id`.

## Obtener todo (paginación)

Los resultados son los más recientes primero y están paginados por cursor.
...
# paginación manual: devuelve next_cursor en cada solicitud
...
Los nombres de campo desconocidos son rechazados (salida `2`) junto con la lista de válidos, una forma sencilla de descubrir nombres de campo.
...
## Seleccionar tu organización (multi-tenant)
...
agenteye --org globex --json sessions --since 24h   # sobrescribe para un único comando
```

Un inicio de sesión en múltiples organizaciones sin `--org` sale con código no cero e imprime las organizaciones disponibles.

## Crear una clave de API para el SDK/collector
...
> **Nota:** Las mutaciones omiten automáticamente su confirmación cuando se usa `--json` o cuando stdin no es un TTY, por lo que los agentes nunca se quedan bloqueados; pasa `--yes`/`-y` para omitirla explícitamente en otros contextos.
...
case "${code:-0}" in
  ...
  5) echo "Permiso faltante (solicita a un administrador evaluations:read)." >&2 ;;
...
| create/update/delete (cualquiera) | el objeto del recurso, o `{"deleted": true, "id"}` en caso de eliminaciones |
| fallo (cualquiera, con `--json`) | `{"error": "...", "exit_code": <n>, "status"?: <http>, "hint"?: "..."}` en stdout |
...
- [Claves de API](/es/agenteye/api-keys): crea y define el alcance de las claves con las que se autentican la CLI, el SDK y el collector.
