...
Claude Code invoca entonces `failproofai --hook PreToolUse` como subproceso antes de cada llamada a una herramienta, enviando un payload JSON por stdin.
...
**Instrucción (cualquier evento excepto Stop):**
```json
{
  "hookSpecificOutput": {
...
`allow(message)` permite que una política envíe contexto informativo a Claude incluso cuando la operación está permitida. El manejador de hooks escribe el siguiente JSON en **stdout** (no en un archivo de configuración — esta es la respuesta del manejador a Claude Code, al igual que las respuestas de denegación e instrucción anteriores):
...
- Cuando múltiples políticas devuelven `allow` con un mensaje, sus mensajes se unen con saltos de línea en una única cadena `additionalContext`
...
- `policyParams` - por clave de política, el primer archivo que la define tiene precedencia completa
- `customPoliciesPath` - el primer archivo que lo define tiene precedencia
- `llm` - el primer archivo que lo define tiene precedencia
...
5. Si el resultado es `deny`, detiene inmediatamente y devuelve esa decisión.
...
- Si se devolvió algún `deny`, emite la respuesta de denegación.
- Si se recopilaron respuestas `instruct`, emite una única respuesta de instrucción con todos los mensajes unidos.
- En caso contrario, emite una respuesta de permiso (stdout vacío, código de salida 0).
...
La coincidencia de patrones dentro de las políticas usa tokens de comandos analizados (argv), no coincidencia de cadenas sin procesar. Esto evita eludir las políticas mediante inyección de operadores de shell (por ejemplo, un patrón para `sudo systemctl status *` no puede saltarse añadiendo `; rm -rf /` al comando).
...
3. Reescribe todas las importaciones `from "failproofai"` a la ruta dist real para que `customPolicies` se resuelva al mismo registro `globalThis`.
...
Las políticas personalizadas se evalúan después de todas las políticas integradas. Un `deny` de una política personalizada sigue cortocircuitando el resto de políticas personalizadas (pero todos los integrados ya se habrán ejecutado en ese punto).
...
- Los componentes de página llaman a `lib/projects.ts` y `lib/log-entries.ts` para leer datos de proyectos/sesiones directamente del sistema de archivos (sin capa API para lecturas).
...
- Componentes cliente solo donde se necesita interactividad (activación de políticas, búsqueda en actividad, visor de registros).
...
│   ├── log-entries.ts            # Analizar el formato JSONL de transcripciones de Claude
