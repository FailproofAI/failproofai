...
Claude Code invoca entonces `failproofai --hook PreToolUse` como subproceso antes de cada llamada a una herramienta, pasando un payload JSON por stdin.
...
El manejador aplica un límite de 1 MB en stdin. Los payloads que superan este límite se descartan y todas las políticas permiten implícitamente.
...
- Cuando múltiples políticas devuelven `allow` con un mensaje, sus mensajes se unen con saltos de línea en un único string `additionalContext`
...
stdin JSON
  ...
  → evaluar todas las políticas (builtins primero, luego personalizadas)
...
`src/hooks/hooks-config.ts` implementa la carga de configuración con tres alcances.
...
Lógica de fusión:
- `enabledPolicies` - unión deduplicada entre los tres archivos
...
- Si se devolvió algún `deny`, emitir la respuesta de deny.
- Si se recopilaron devoluciones `instruct`, emitir una única respuesta instruct con todos los mensajes unidos.
...
La coincidencia de patrones dentro de las políticas usa tokens de comando parseados (argv), no coincidencia de cadenas sin procesar. Esto evita la evasión mediante inyección de operadores de shell (por ejemplo, un patrón para `sudo systemctl status *` no puede eludirse añadiendo `; rm -rf /` al comando).
...
Las políticas personalizadas se evalúan después de todas las políticas integradas. Un `deny` de una política personalizada aún cortocircuita las políticas personalizadas posteriores (pero en ese punto todas las integradas ya se han ejecutado).

---

## Registro de actividad

Después de cada evento de hook, el manejador añade una línea JSONL a `~/.failproofai/hook-activity/current.jsonl`, que rota a `page-<timestamp>-<seq>.jsonl` una vez que alcanza una página:
...
app/
  ...
  project/[name]/page.tsx     ← Componente de servidor: lista sesiones en un proyecto
...
- El visor de sesiones parsea el formato de transcripción JSONL de Claude y renderiza una línea de tiempo de mensajes y llamadas a herramientas.
...
- React Server Components para páginas de lectura: carga inicial más rápida, sin bundle cliente para la obtención de datos.
- Componentes cliente solo donde se necesita interactividad (toggles de políticas, búsqueda de actividad, visor de registros).
...
│   ├── hooks-config.ts           # Carga de configuración multi-alcance
...
│   ├── log-entries.ts            # Parsear formato JSONL de transcripción Claude
│   ├── paths.ts                  # Resolver rutas del sistema
│   └── ...
├── components/                   # Componentes de interfaz React compartidos
