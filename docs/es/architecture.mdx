...
Al ejecutar `failproofai policies --install`, se escriben entradas como esta en `~/.claude/settings.json`:
...
Claude Code invoca entonces `failproofai --hook PreToolUse` como subproceso antes de cada llamada a herramienta, enviando un payload JSON por stdin.
...
El manejador aplica un límite de 1 MB para stdin. Los payloads que superen este límite se descartan y todas las políticas permiten implícitamente.
...
- Cuando múltiples políticas devuelven `allow` con mensaje, sus mensajes se concatenan con saltos de línea en una sola cadena `additionalContext`
...
stdin JSON
  ...
  → registrar políticas builtin habilitadas con parámetros resueltos
  ...
  → evaluar todas las políticas (primero builtins, luego personalizadas)
      → el primer deny interrumpe el proceso
      → las decisiones instruct se acumulan
      → los mensajes allow se acumulan
  → escribir decisión JSON en stdout
  → persistir evento en ~/.failproofai/hook-activity/current.jsonl
  → salir
```
...
1. Se busca el esquema `params` de la política (si tiene uno).
2. Se lee `policyParams[policy.name]` desde la configuración fusionada.
3. Se fusionan los valores proporcionados por el usuario sobre los valores por defecto del esquema para producir `ctx.params`.
4. Se llama a `policy.fn(ctx)` con el contexto resuelto.
5. Si el resultado es `deny`, se detiene inmediatamente y se devuelve esa decisión.
6. Si el resultado es `instruct`, se acumula el mensaje y se continúa.
7. Si el resultado es `allow`, se continúa con la siguiente política.

Tras ejecutar todas las políticas:
- Si se devolvió algún `deny`, se emite la respuesta de deny.
- Si se recopilaron respuestas `instruct`, se emite una única respuesta instruct con todos los mensajes concatenados.
- En caso contrario, se emite una respuesta allow (stdout vacío, exit 0).
...
Las políticas que aceptan `params` declaran un `PolicyParamsSchema` con tipos y valores por defecto para cada parámetro. El evaluador de políticas inyecta los valores resueltos en `ctx.params` antes de llamar a `fn`. Las funciones de política leen `ctx.params` sin comprobaciones de nulo porque los valores por defecto siempre se aplican primero.

La coincidencia de patrones dentro de las políticas usa tokens de comando parseados (argv), no coincidencia de cadenas sin procesar. Esto impide la evasión mediante inyección de operadores de shell (por ejemplo, un patrón para `sudo systemctl status *` no puede eludirse añadiendo `; rm -rf /` al comando).
...
1. Lee `customPoliciesPath` desde la configuración; se omite si no está presente.
2. Resuelve a ruta absoluta; verifica que el archivo exista.
3. Reescribe todas las importaciones `from "failproofai"` a la ruta real de dist para que `customPolicies` resuelva al mismo registro `globalThis`.
...
7. Elimina todos los archivos temporales en un bloque `finally`.

Ante cualquier error (archivo no encontrado, error de sintaxis, fallo en la importación), el error se registra en `~/.failproofai/hook.log` y el cargador devuelve un array vacío. Las políticas builtin no se ven afectadas.

Las políticas personalizadas se evalúan después de todas las políticas builtin. Un `deny` de una política personalizada sigue interrumpiendo el procesamiento de las políticas personalizadas restantes (aunque para ese momento todos los builtins ya habrán sido ejecutados).

---

## Registro de actividad

Tras cada evento de hook, el manejador añade una línea JSONL a `~/.failproofai/hook-activity/current.jsonl`, que rota a `page-<timestamp>-<seq>.jsonl` una vez que alcanza el tamaño de página:
...
app/
  ...
  projects/page.tsx           ← Componente servidor: lista todos los proyectos de Claude
...
- Componentes cliente solo donde se requiere interactividad (toggles de políticas, búsqueda de actividad, visor de logs).
...
│   ├── hook-activity-store.ts    # Persistir actividad en hook-activity/
...
│   ├── log-entries.ts            # Parsear formato JSONL de transcripciones de Claude
