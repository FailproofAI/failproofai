...
Claude Code luego invoca `failproofai --hook PreToolUse` como subproceso antes de cada llamada a herramienta, enviando un payload JSON por stdin.
...
**Evento Stop con instruct:**
- Código de salida: `2`
- Razón escrita en stderr (no en stdout)
...
- `policyParams` - por clave de política, gana el primer archivo que la defina en su totalidad
- `customPoliciesPath` - gana el primer archivo que lo defina
- `llm` - gana el primer archivo que lo defina

El panel de control web usa `readHooksConfig()` (solo global) para leer y escribir, ya que no se invoca con un cwd de proyecto.
...
1. Busca el esquema de `params` de la política (si tiene uno).
2. Lee `policyParams[policy.name]` de la configuración fusionada.
...
- De lo contrario, emite una respuesta de allow (stdout vacío, exit 0).
...
Las políticas que aceptan `params` declaran un `PolicyParamsSchema` con tipos y valores predeterminados para cada parámetro. El evaluador de políticas inyecta los valores resueltos en `ctx.params` antes de llamar a `fn`. Las funciones de política leen `ctx.params` sin verificaciones de nulo porque los valores predeterminados siempre se aplican primero.

La coincidencia de patrones dentro de las políticas usa tokens de comando analizados (argv), no comparación de cadenas en bruto. Esto evita elusiones mediante inyección de operadores de shell (por ejemplo, un patrón para `sudo systemctl status *` no puede eludirse añadiendo `; rm -rf /` al comando).
...
2. Resuelve la ruta absoluta; verifica que el archivo existe.
3. Reescribe todas las importaciones `from "failproofai"` a la ruta dist real para que `customPolicies` resuelva al mismo registro de `globalThis`.
...
Las políticas personalizadas se evalúan después de todas las políticas integradas. Un `deny` de una política personalizada sigue cortocircuitando las demás políticas personalizadas (pero todos los integrados ya se habrán ejecutado en ese momento).
...
app/
  ...
  projects/page.tsx           ← Componente de servidor: lista todos los proyectos de Claude
...
- Componentes de cliente solo donde se necesita interactividad (activación de políticas, búsqueda en actividad, visor de logs).
...
│   └── llm-client.ts             # Cliente API LLM (para políticas basadas en IA)
