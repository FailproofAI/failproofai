...
Este es el comando registrado en el `settings.json` de Claude Code por `failproofai policies --install`. Normalmente no se llama directamente.
...
|-----------------|----------|--------|
| `0` | `allow` | Permitir la acción |
| `1` | `deny` | Bloquear la acción — Claude recibe el motivo del rechazo |
| `2` | `instruct` | Inyectar orientación en el contexto de Claude |
...
| **Ciclo de vida de la sesión** | `SessionStart`, `SessionEnd`, `Stop`, `StopFailure` |
