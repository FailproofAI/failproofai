...
Las políticas personalizadas te permiten escribir reglas para cualquier comportamiento del agente: aplicar convenciones del proyecto, prevenir la deriva, bloquear operaciones destructivas, detectar agentes atascados o integrarte con Slack, flujos de aprobación y más. Usan el mismo sistema de eventos hook y las decisiones `allow`, `deny`, `instruct` que las políticas integradas.
...
Coloca archivos `*policies.{js,mjs,ts}` en `.failproofai/policies/` y se cargarán automáticamente, sin flags ni cambios de configuración. Funciona como los git hooks: deposita un archivo y listo.
...
**Cómo funciona:**
- Se escanean los directorios del proyecto y del usuario (unión — no gana el primer ámbito)
...
<Tip>
Las políticas basadas en convenciones son la forma más sencilla de establecer un estándar de calidad para tu organización. Incluye `.failproofai/policies/` en git y cada miembro del equipo recibirá las mismas reglas automáticamente, sin configuración individual. A medida que el equipo descubra nuevos modos de fallo, añade una política y haz push. Con el tiempo, estas se convierten en un estándar de calidad vivo que mejora con cada contribución.
</Tip>
...
customPolicies.add({
  ...
  description?: string;                 // mostrado en la salida de `failproofai policies`
...
- **Explicaciones de fallo abierto:** `allow("GitHub CLI not installed, skipping CI check.")` — indica a Claude por qué se omitió una verificación para que tenga contexto completo
...
<Note>
El primer `deny` cortocircuita todas las políticas subsiguientes. Todos los mensajes `instruct` se acumulan y se entregan juntos.
</Note>
...
Se resuelven todas las importaciones relativas alcanzables desde el archivo de entrada. Esto se implementa reescribiendo las importaciones `from "failproofai"` a la ruta dist real y creando archivos `.mjs` temporales para garantizar la compatibilidad con ESM.
...
Las políticas personalizadas son **fail-open**: los errores nunca bloquean las políticas integradas ni hacen que el manejador de hook falle.
...
| Directorio de convención ausente | No se ejecutan políticas de convención; sin error |
...
### Usar los ejemplos de archivos explícitos
...
No se necesita comando de instalación — los archivos se detectan automáticamente en el próximo evento hook.