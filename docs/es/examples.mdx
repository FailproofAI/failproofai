...
Failproof AI se integra con Claude Code a través de su [sistema de hooks](https://docs.anthropic.com/en/docs/claude-code/hooks). Al ejecutar `failproofai policies --install`, registra comandos de hook en el archivo `settings.json` de Claude Code que se disparan en cada llamada a una herramienta.
...
<Steps>
  ...
  <Step title="Configurar hooks en tu agente">
    Pasa comandos de hook al crear el proceso de tu agente. Los hooks se disparan de la misma manera que en Claude Code — mediante stdin/stdout JSON:
...
- `block-curl-pipe-sh` - bloquea la descarga y ejecución de scripts remotos en el shell

---

## Prevenir la filtración de secretos

Impide que los agentes vean o expongan credenciales en la salida de las herramientas.
...
Confirma una configuración a nivel de proyecto para que todos los desarrolladores de tu equipo compartan las mismas políticas.
...
## Crear un estándar de calidad organizacional con políticas de convenciones
...
<Steps>
  ...
  <Step title="Seguir mejorando">
    A medida que tu equipo detecte nuevos fallos, añade políticas y haz push. Todos recibirán la actualización en su próximo `git pull`. Estas políticas se convierten en un estándar de calidad vivo que crece con tu equipo.
  </Step>
</Steps>
...
| Archivo | Qué muestra |
|---------|-------------|
| `policies-basic.js` | Políticas iniciales: bloquear escrituras en producción, force-push y scripts ejecutados desde tuberías |
