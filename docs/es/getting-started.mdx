...
<Steps>
  ...
  <Step title="Ejecutar tu agente">
    Inicia Claude Code de la manera habitual. Si el agente intenta algo arriesgado, failproofai lo intercepta automáticamente. Déjalo ejecutándose de forma desatendida y revisa lo que ocurrió en el panel de control.
  </Step>
</Steps>
...
Cada política devuelve una de estas tres decisiones:
...
<Tip>
Confirma `.failproofai/policies/` en tu repositorio para que todo el equipo comparta los mismos estándares. A medida que el equipo descubra nuevos modos de fallo, añade políticas y súbelas — todos recibirán la actualización en su próximo `git pull`. Con el tiempo, estas políticas se convierten en un estándar de calidad vivo que no deja de mejorar.
</Tip>
...
| `~/.failproofai/logs/` | Registros de depuración para errores en hooks personalizados |
| `.failproofai/policies-config.json` | Configuración por proyecto (confirmada en el repositorio) |
| `.failproofai/policies-config.local.json` | Anulaciones personales (ignoradas por git) |
