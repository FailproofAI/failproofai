...
Las pruebas E2E invocan el binario real de `failproofai` como subproceso, le envían un payload JSON por stdin y verifican la salida en stdout junto con el código de salida. Esto prueba el flujo de integración completo que utiliza Claude Code.
...
`createFixtureEnv()` registra la limpieza en `afterEach` de forma automática.
