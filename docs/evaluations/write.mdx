...
description: "Describe what to measure and let the assistant draft a hosted Python evaluation, or write the code yourself."
icon: "file-pen-line"
---

Hosted evaluations are small, deterministic Python, written in the dashboard and run on Failproof AI's evaluator fleet. They count and compare: how many tool calls, how many errors, how long a session took.

For questions that need the conversation to be *understood* — was the answer correct, was the reply rude, did the agent follow a policy — write an [LLM judge](/evaluations/judge) instead. It is authored in the same place, from a description of what good looks like.

Anything needing a package, a secret, or your own network runs in [your own worker](#write-it-in-your-own-worker).
...
When an evaluation needs a package, a secret, the network, or a model you host yourself, write it with the [Evaluator SDK](/reference/evaluator-sdk) and run it on your own infrastructure. It uses the same result types, and its results appear beside hosted ones, tagged **customer**:
