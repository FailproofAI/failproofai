...
Extrayez les données de sessions, d'événements et d'évaluations (et déclenchez des réévaluations) directement depuis un script ou un agent de codage, avec du JSON propre sur stdout qui s'intègre directement dans `jq`. Ces recettes transforment les données de Failproof AI Observability en quelque chose qu'un utilisateur en terminal ou un agent de codage IA (Claude Code, Cursor) peut interroger et automatiser, sans avoir à cliquer dans le tableau de bord.

Les modèles ci-dessous sont prêts à copier-coller pour la CLI Failproof AI Observability (`agenteye`). Pour l'installation, l'authentification et la liste complète des options, consultez [CLI](/fr/agenteye/cli) ; exécutez `agenteye -h` ou `agenteye <command> -h` pour l'aide intégrée.
...
2. **Passez `--json` à chaque fois que vous analysez la sortie.** Les données vont sur **stdout** en JSON ; les statuts et erreurs pour les humains vont sur **stderr**, ce qui maintient stdout propre pour le piping dans `jq`.
3. **Branchez-vous sur le code de sortie**, pas sur le texte stderr : `0` ok · `1` erreur inattendue · `2` mauvais arguments · `3` impossible d'atteindre le tableau de bord · `4` non connecté ou expiré · `5` permission manquante · `6` ressource introuvable.
...
`whoami` n'échoue jamais en cas de session manquante ou expirée ; il retourne `logged_in:false` à la place, ce qui permet à un agent de tester l'état d'authentification en toute sécurité. (Il peut tout de même sortir avec un code non nul si aucune URL de base n'est définie ou si le tableau de bord est inaccessible.)
...
## Trouver les sessions échouées ou à faible score
...
Le filtrage par score se fait sur **`evals`**, pas sur `sessions`. `--score KEY:MIN..MAX` est répétable et combiné en AND ; chaque borne est optionnelle (`..0.5` signifie ≤ 0.5, `0.9..` signifie ≥ 0.9). Vous pouvez passer jusqu'à 20 filtres de score par requête ; au-delà, le serveur renvoie HTTP 400. `sessions` partage les filtres `--env`, `--status`, `--agent-id`, `--session-id` et les plages temporelles avec `evals`, mais n'a pas de `--score`.

## Lire une session de bout en bout

Il n'existe pas de commande unique `session show`. Combinez la piste d'événements avec l'évaluation de la session :
...
Les noms de champs inconnus sont rejetés (sortie `2`) avec la liste des champs valides, un moyen pratique de découvrir les noms de champs.
...
> **Note :** Les mutations ignorent automatiquement leur invite de confirmation sous `--json` ou lorsque stdin n'est pas un TTY, ce qui évite tout blocage des agents ; passez `--yes`/`-y` pour l'ignorer explicitement ailleurs.
...
- [Compétence CLI pour agent](/fr/agenteye/cli-skill) : regroupez ces recettes sous forme de compétence que votre agent de codage peut charger.
- [Clés API](/fr/agenteye/api-keys) : créez et délimitez les clés avec lesquelles la CLI, le SDK et le collecteur s'authentifient.
