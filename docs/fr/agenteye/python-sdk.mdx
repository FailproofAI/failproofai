---
title: "Python SDK"
description: "Observez exactement ce que vos agents IA ont fait en production : chaque exécution d'agent, appel d'outil, requête au modèle, hook et intervention humaine."
---


Observez exactement ce que vos agents IA ont fait en production : chaque exécution d'agent, appel d'outil, requête au modèle, hook et intervention humaine. Le SDK Python d'observabilité Failproof AI enregistre cette trace depuis le code de votre agent afin que vous puissiez déboguer, auditer et évaluer ce qui s'est passé. Utilisez-le chaque fois que vous souhaitez que Failproof AI Observability observe vos agents.

En coulisses, le SDK écrit des événements structurés dans des fichiers JSONL locaux, et le daemon collecteur les récupère et les envoie automatiquement à la plateforme. Vous n'avez pas à gérer ces fichiers vous-même.
...
Le SDK est distribué aux clients sous forme de wheel privé plutôt que depuis un index de packages public. Votre processus d'intégration explique comment l'obtenir, l'installer et le figer — contactez votre interlocuteur Failproof AI si vous avez besoin d'un accès.
...
Vous préférez laisser un agent de codage réaliser l'intégration complète ? Le [Python SDK Agent Skill](/fr/agenteye/python-sdk-skill) connaît le chemin d'installation, planifie les points d'instrumentation, les écrit et vérifie que les événements arrivent bien.
...
Enveloppez les appels d'outils de la même façon avec `tool_use` et `tool_result`, en réutilisant un même `tool_call_id` pour la paire.

Voici à quoi ressemblent ces événements une fois qu'ils arrivent sur le tableau de bord, avec un code couleur par type et des filtres par environnement, agent et session :

![Le flux d'événements en direct, avec un code couleur par type et des filtres par environnement, agent et session](/agenteye/images/events-stream.png)
...
À appeler une seule fois avant tout appel à `event.*`. Sans danger à omettre ; les valeurs par défaut fonctionnent immédiatement. Tous les arguments sont uniquement par mot-clé ; passez-les par nom comme indiqué ci-dessus.
...
ainsi une seule variable d'environnement `AGENTEYE_HOME` configure la file d'événements partagée pour
le SDK et le collecteur.
...
**Priorité :** `configure(environment=...)` l'emporte sur la variable d'environnement. Si aucun des deux n'est défini, la valeur par défaut est `"dev"`.
...
> **Avertissement :** Les valeurs d'environnement ne doivent pas contenir de virgule `,` littérale. Les filtres du tableau de bord utilisent une sélection multiple séparée par des virgules dans l'URL (`?environment=prod,staging`), donc un environnement nommé `prod,blue` serait divisé en deux valeurs. Les événements dont l'environnement contient une virgule sont rejetés à l'ingestion.

---

## Données et confidentialité

Le SDK enregistre uniquement les champs que vous passez explicitement. Les prompts, messages, entrées et sorties d'outils, et le contenu du modèle sont capturés uniquement parce que vous les transmettez à un appel `event.*`. Rien n'est lu depuis votre processus ni capturé implicitement. Tout champ que vous ne renseignez pas est omis de l'événement ; il n'est pas écrit sur disque.
...
![Le graphe d'exécution de type git d'une session à côté de sa timeline d'événements, reconstruit à partir des événements appariés, avec le panneau de répartition outil/modèle/hook](/agenteye/images/session-detail.png)
...
Émis lorsqu'un agent commence son travail.
...
Les entrées `messages` acceptent soit un `content` sous forme de chaîne simple, soit un `content` sous forme de liste de blocs à la façon d'Anthropic. Les paramètres d'échantillonnage (`temperature`, `max_tokens`, etc.) peuvent être passés en kwargs supplémentaires.
...
`content` accepte soit une chaîne simple (fournisseurs génériques), soit une liste de blocs de contenu à la façon d'Anthropic. Les appels d'outils se trouvent dans `content` sous forme de blocs `{"type": "tool_use", ...}`, sans champ `tool_calls` séparé.
...
## Événements de supervision humaine

Les événements de supervision humaine vous donnent une visibilité sur les moments où une personne intervient dans l'exécution de l'agent (attente d'approbation, fourniture d'une entrée, mise en pause ou arrêt de l'agent). Ils vous permettent de mesurer le temps de réponse humain (le SDK calcule automatiquement `duration_ms` sur les événements appariés), d'auditer qui a mis en pause ou interrompu un agent, et de construire des workflows d'approbation et de supervision qui apparaissent dans le tableau de bord.

### `event.human_wait()`

Émis lorsque l'agent suspend son exécution en attendant qu'un humain fournisse une entrée. À associer avec `human_input` ; le SDK calcule automatiquement `duration_ms` (temps que l'humain a mis à répondre).
...
Émis lorsqu'un humain fournit une entrée et que l'agent reprend son exécution. Corrélé avec `human_wait` via `input_id`. `duration_ms` est calculé automatiquement et ne doit pas être passé par l'appelant.
...
Émis lorsqu'un humain met activement l'agent en pause (par ex. via un contrôle du tableau de bord). L'agent est suspendu mais pas arrêté.
...
Conservez les charges utiles en JSON structuré lorsque vous souhaitez interroger leurs champs. Les valeurs que JSON ne prend pas nativement en charge — telles que les datetimes, UUIDs, décimaux, ensembles, octets ou objets de modèle — sont converties en chaînes pour que l'enregistrement continue de manière sécurisée.
...
- [Sessions](/fr/agenteye/sessions) : découvrez comment les événements appariés reconstruisent chaque exécution d'agent sous forme de graphe d'exécution et de timeline.