...
2. **Agent Monitor (Tableau de bord)** — Une application web Next.js pour surveiller les sessions de l'agent et gérer les politiques.

Les deux sous-systèmes partagent des fichiers de configuration dans `~/.failproofai/` et le répertoire `.failproofai/` du projet, mais s'exécutent comme des processus distincts et ne communiquent qu'à travers le système de fichiers.
...
Claude Code invoque ensuite `failproofai --hook PreToolUse` comme sous-processus avant chaque appel d'outil, en transmettant une charge utile JSON sur stdin.
...
Le gestionnaire impose une limite de 1 Mo sur stdin. Les charges utiles dépassant cette limite sont ignorées et toutes les politiques autorisent implicitement.
...
- Code de sortie : `2`
- Raison écrite sur stderr (et non stdout)
...
- Lorsque plusieurs politiques retournent `allow` avec un message, leurs messages sont joints par des retours à la ligne en une seule chaîne `additionalContext`
...
stdin JSON
  ...
  → évaluation de toutes les politiques (intégrées en premier, puis personnalisées)
      → le premier refus court-circuite l'évaluation
...
L'ensemble du processus s'exécute en moins de 100 ms pour les charges utiles typiques, sans aucun appel LLM.
...
Le tableau de bord web utilise `readHooksConfig()` (global uniquement) pour la lecture et l'écriture, car il n'est pas invoqué avec un répertoire de projet.
...
- Sinon, émission d'une réponse d'autorisation (stdout vide, code de sortie 0).
...
La correspondance de motifs à l'intérieur des politiques utilise des tokens de commande analysés (argv), et non une correspondance de chaîne brute. Cela empêche les contournements par injection d'opérateurs shell (par exemple, un motif pour `sudo systemctl status *` ne peut pas être contourné en ajoutant `; rm -rf /` à la commande).
...
3. Réécriture de tous les imports `from "failproofai"` vers le chemin dist réel afin que `customPolicies` résolve vers le même registre `globalThis`.
...
En cas d'erreur (fichier introuvable, erreur de syntaxe, échec d'import), l'erreur est enregistrée dans `~/.failproofai/hook.log` et le chargeur retourne un tableau vide. Les politiques intégrées ne sont pas affectées.
...
Après chaque événement de hook, le gestionnaire ajoute une ligne JSONL à `~/.failproofai/hook-activity/current.jsonl`, qui bascule vers `page-<timestamp>-<seq>.jsonl` une fois qu'elle atteint la taille d'une page :
...
│   ├── log-entries.ts            # Analyse du format JSONL de transcription Claude
│   ├── paths.ts                  # Résolution des chemins système
│   └── ...
├── components/                   # Composants React UI partagés
├── contexts/                     # Fournisseurs de contexte React (thème, rafraîchissement auto, télémétrie)
