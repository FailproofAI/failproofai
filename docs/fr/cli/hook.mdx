...
Lit un payload JSON depuis stdin, évalue toutes les politiques activées, puis se termine avec un code indiquant la décision :

| Code de sortie | Décision | Effet |
|----------------|----------|-------|
| `0` | `allow` | Autoriser l'action |
| `1` | `deny` | Bloquer l'action — Claude reçoit le motif du refus |
