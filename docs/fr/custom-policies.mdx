...
Déposez des fichiers `*policies.{js,mjs,ts}` dans `.failproofai/policies/` et ils sont chargés automatiquement — sans indicateurs ni modifications de configuration. Cela fonctionne comme les git hooks : déposez un fichier, ça marche tout de suite.
...
- Les répertoires de projet et utilisateur sont tous deux analysés (union — pas de priorité par portée)
...
# Supprimer tous les chemins de politiques personnalisées explicites de la config
failproofai policies --uninstall --custom
```

Les chemins absolus résolus sont stockés dans `policies-config.json` sous `customPoliciesPaths`. Répétez `--custom` pour configurer plusieurs fichiers. Les configurations existantes utilisant le champ legacy `customPoliciesPath` continuent de fonctionner. Les fichiers sont rechargés à chaque événement de hook — il n'y a pas de mise en cache entre les événements.

Chaque politique enregistrée apparaît avec son propre bouton dans le tableau de bord. Désactiver une politique enregistre son identifiant qualifié par sa source dans `disabledCustomPolicies` ; le fichier et ses autres politiques continuent de se charger, tandis que la politique désactivée est exclue avant la correspondance d'événements. Les noms de politiques dupliqués entre fichiers ont des boutons indépendants.
...
customPolicies.add({
  ...
  match?: { events?: HookEventType[] }; // filtre par type d'événement ; omettez pour tout matcher
...
| `deny(message)` | Bloquer l'opération | L'agent ne doit pas effectuer cette action |
| `instruct(message)` | Ajouter du contexte sans bloquer | Donner à l'agent un contexte supplémentaire pour rester dans le bon sens |
...
<Tip>
Vous pouvez ajouter des instructions supplémentaires à n'importe quel message `deny` ou `instruct` en ajoutant un champ `hint` dans `policyParams` — sans modification de code. Cela fonctionne également pour les politiques personnalisées (`custom/`), de convention de projet (`.failproofai-project/`) et de convention utilisateur (`.failproofai-user/`). Consultez [Configuration → hint](/fr/configuration#hint-cross-cutting) pour plus de détails.
</Tip>

### Messages allow informatifs

`allow(message)` autorise l'opération **et** envoie un message informatif à Claude. Le message est transmis comme `additionalContext` dans la réponse stdout du gestionnaire de hook — le même mécanisme que `instruct`, mais sémantiquement différent : c'est une mise à jour de statut, pas un avertissement.
...
Cas d'usage :
- **Confirmations de statut :** `allow("All CI checks passed.")` — indique à Claude que tout est bon
...
|-------|--------------|----------------------|
| `PreToolUse` | Avant qu'Claude exécute un outil | L'entrée de l'outil (ex. `{ command: "..." }` pour Bash) |
...
Omettez entièrement `match` pour se déclencher sur chaque type d'événement.
...
| Échec | Comportement |
|---------|----------|
| `customPoliciesPath` non défini | Aucune politique personnalisée explicite ne s'exécute ; les politiques de convention et intégrées continuent normalement |
| Fichier introuvable | Avertissement enregistré dans `~/.failproofai/hook.log` ; les politiques intégrées continuent |
| Erreur de syntaxe/import (explicite) | Erreur enregistrée dans `~/.failproofai/hook.log` ; politiques personnalisées explicites ignorées |
| Erreur de syntaxe/import (convention) | Erreur enregistrée ; ce fichier ignoré, les autres fichiers de convention continuent de se charger |
...
| Fichier | Contenu |
|------|----------|
| `examples/policies-basic.js` | Cinq politiques de démarrage couvrant les modes d'échec d'agent les plus courants |
...
Aucune commande d'installation requise — les fichiers sont automatiquement pris en compte au prochain événement de hook.