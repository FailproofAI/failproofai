...
Des exemples prêts à l'emploi pour les scénarios courants. Chacun montre comment effectuer l'installation et ce à quoi vous pouvez vous attendre.
...
<Steps>
  ...
  <Step title="Configurer les hooks dans votre agent">
    Transmettez les commandes de hook lors de la création de votre processus d'agent. Les hooks se déclenchent de la même façon que dans Claude Code — via JSON sur stdin/stdout :
...
- `block-curl-pipe-sh` — bloque l'envoi de scripts distants vers un shell
...
Committez une configuration au niveau du projet afin que chaque développeur de votre équipe bénéficie des mêmes politiques.
...
Le répertoire [`examples/`](https://github.com/failproofai/failproofai/tree/main/examples) dans le dépôt contient :

| Fichier | Ce qu'il montre |
|---------|-----------------|
| `policies-basic.js` | Politiques de base — blocage des écritures en production, des force-push et des scripts pipés |
| `policies-notification.js` | Alertes Slack pour les notifications d'inactivité et la fin de session |
| `policies-advanced/index.js` | Imports transitifs, hooks asynchrones, nettoyage des sorties PostToolUse, gestion de l'événement Stop |