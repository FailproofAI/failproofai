---
title: Exemples
description: "Comment configurer des hooks pour Claude Code et le SDK Agents"
icon: book-open
---
...
## Configurer des hooks pour le SDK Agents

Si vous développez avec le [SDK Agents](https://docs.anthropic.com/en/docs/agents-sdk), vous pouvez utiliser le même système de hooks de manière programmatique.
...
<Steps>
  ...
  <Step title="Configurer les hooks dans votre agent">
    Transmettez les commandes de hook lors de la création de votre processus agent. Les hooks se déclenchent de la même manière que dans Claude Code — via JSON sur stdin/stdout :
...
Évitez que les agents ne voient ou ne divulguent des identifiants dans la sortie des outils.
...
Committez une configuration au niveau du projet afin que tous les développeurs de votre équipe appliquent les mêmes politiques.
...
Tous les membres de l'équipe ayant failproofai installé récupéreront automatiquement ces règles.

---

## Établir un standard qualité à l'échelle de l'organisation avec des politiques de convention

La configuration la plus impactante : committez `.failproofai/policies/` dans votre dépôt avec des politiques adaptées à votre projet. Tous les membres de l'équipe les reçoivent automatiquement — sans commandes d'installation ni modifications de configuration.
...
|---------|-------------------|
| `policies-basic.js` | Politiques de base — bloquer les écritures en production, les force-push et les scripts via pipe |
