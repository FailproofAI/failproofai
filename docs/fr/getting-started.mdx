...
<Steps>
  ...
  <Step title="Valider dans git">
    ...
    Chaque membre de l'équipe qui a failproofai installé récupère automatiquement ces politiques. Aucune configuration individuelle n'est nécessaire.
  </Step>
</Steps>
...
Toutes les configurations et les journaux restent sur votre machine :

| Chemin | Contenu |
|--------|---------|
| `~/.failproofai/policies-config.json` | Configuration globale des politiques |
| `~/.failproofai/policies/` | Vos propres politiques — déposez des fichiers `*-policies.mjs`, aucune configuration requise |
...
Supprime les entrées de hooks de `~/.claude/settings.json`. Les fichiers de configuration dans `~/.failproofai/` sont conservés.
