...
__tests__/e2e/
  helpers/
    hook-runner.ts      # Lance le binaire, transmet le JSON du payload, capture le code de sortie + stdout + stderr
  ...
  hooks/
    ...
    config-scopes.e2e.test.ts      # Fusion de configuration sur les scopes project/local/global
...
// env.cwd    - répertoire temporaire ; à passer comme payload.cwd pour récupérer .failproofai/policies-config.json
