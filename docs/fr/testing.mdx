...
failproofai dispose de deux suites de tests : **tests unitaires** (rapides, avec mocks) et **tests de bout en bout** (invocations réelles de sous-processus).
...
Les tests E2E invoquent le vrai binaire `failproofai` en tant que sous-processus, transmettent un payload JSON via stdin et vérifient la sortie stdout ainsi que le code de retour. Cela teste le chemin d'intégration complet qu'utilise Claude Code.
...
__tests__/e2e/
  helpers/
    hook-runner.ts      # Lance le binaire, transmet le JSON du payload, capture le code de retour + stdout + stderr
  ...
  hooks/
    ...
    config-scopes.e2e.test.ts      # Fusion de la configuration entre les scopes projet/local/global
...
// env.home   - répertoire home isolé ; évite toute fuite de ~/.failproofai réel
...
- `testTimeout: 20_000` - 20 s par test (démarrage du binaire + évaluation des hooks)

Le pool `forks` est important : les workers basés sur les threads partagent `globalThis`, ce qui peut interférer avec les tests qui lancent des sous-processus. Les forks basés sur des processus évitent ce problème.

---

## Intégration continue

L'exécution complète CI (`bun run lint && bunx tsc --noEmit && bun run test:run && bun run build`) doit réussir avant toute fusion. La suite E2E s'exécute en tant que job CI distinct, en parallèle.
