...
## התחבר ל-Failproof daemon

<Tabs>
  <Tab title="Dashboard">
    1. עבור אל **Admin → Keys** וצור מפתח עם `events:add`.
    2. [התחבר ל-Failproof daemon אל Cloud](/he/start/setup#connect-a-machine-to-cloud) במכונת הסוכן.
    3. הפעל הפעלה אחת מעוקבת, ואז מצא את המזהה המדויק שלה תחת **Observe → Events**.
    4. עבור אל **Observe → Sessions**, בחר את אותה סביבה, ופתח את ה-trace שנבנה מחדש.
...
קרא ל-`configure()` פעם אחת בעת הפעלת התהליך. כל קריאת event היא keyword-only ודורשת `session_id` ו-`agent_id` יציבים.
...
פלוט `agent_start` פעם אחת לכל שחקן. לסוכנים משניים, השתמש מחדש ב-`session_id` של ההורה, תן לכל שחקן `agent_id` ייחודי, וקבע את `parent_id` ל-**agent ID** של ההורה, לא ל-session ID.
...
שלח דוא"ל ל-[support@befailproof.ai](mailto:support@befailproof.ai) כדי לעצב ולאמת את האינטגרציה הזו עבור זמן ההרצה שלך.