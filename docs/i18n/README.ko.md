...
Claude Code 및 Codex에 연결됩니다. 루프, 위험한 동작, 시크릿 유출을
인시던트가 되기 전에 차단합니다. 레이턴시 제로. 로컬에서 실행됩니다.
...
30개의 내장 정책이 즉시 활성화됩니다. 대시보드는 `localhost:8020`에서 확인하세요. `FAILPROOFAI_NO_FIRST_RUN=1`로 최초 실행 프롬프트를 비활성화할 수 있습니다.
...
`.failproofai/policies/` 디렉토리에 파일을 추가하면 자동으로 로드됩니다 — 별도의 플래그 설정이 필요 없습니다.
...
| [내장 정책](https://docs.befailproof.ai/built-in-policies) | 파라미터가 포함된 30개의 정책 전체 |
...
[Commons Clause](https://commonsclause.com/)가 포함된 MIT — 내부 및 개인 사용은 무료이며, failproofai 자체의 상업적 재판매에는 별도의 계약이 필요합니다. 전체 내용은 [LICENSE](../../LICENSE)를 참조하세요.
...
> **시작 전에 빌드를 먼저 실행하세요.** `bun install && bun run build`를 먼저 실행해야 합니다. 이 저장소는
> failproofai 자체의 훅을 스스로에게 적용하며, `failproofai` 임포트를 컴파일된 `dist/` 번들을 기준으로 해석합니다 — 빌드 없이는 `Cannot find package 'failproofai'`
> 훅 오류가 발생합니다. `src/` 변경 후 다시 빌드하세요. 자세한 내용은
