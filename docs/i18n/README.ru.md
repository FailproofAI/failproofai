...
**Разрешение сбоев во время выполнения для кодирующих агентов.**
Интегрируется с Claude Code и Codex. Останавливает циклы, опасные действия и утечки секретов
...
| `block-work-on-main` | Коммиты, слияния, перебазирования в `main` / `master` |
...
Поместите файл в `.failproofai/policies/` — он загрузится автоматически без дополнительных флагов.
...
Каждый вызов инструмента, который делает ваш агент, записывается локально. Панель управления показывает, что было запущено,
что было заблокировано и что политика сказала агенту — чтобы вы не гадали,
...
> **Соберите перед началом.** Сначала запустите `bun install && bun run build`. Этот репозиторий использует собственные хуки failproofai на себе, и они разрешают импорт `failproofai` против скомпилированного пакета `dist/` — без сборки вы получите ошибки хуков `Cannot find package 'failproofai'`. Пересоберите после изменения `src/`. См.
