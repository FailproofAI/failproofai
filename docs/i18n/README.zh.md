...
框架——包括 Claude Code 和 Codex 等编码 CLI，Hermes 等聊天网关，以及 OpenClaw 等自托管助手——捕获每一次运行，并在危险工具调用执行之前将其拦截。内置 45 条策略，零延迟，本地运行。
...
→ [全部 45 条内置策略](https://docs.befailproof.ai/policies/packs)
