...
description: "Configura i valori operativi specifici della distribuzione e fai scelte deliberate sui dati dell'agente."
icon: "lock-keyhole"
---
...
Le credenziali di ingest locali vengono archiviate separatamente dalle impostazioni daemon non segrete e scritte con autorizzazioni restrittive. Le chiavi API devono comunque essere gestite come segreti di produzione.
...
| `alerts.email_default_recipients` | `[]` | Destinatari predefiniti quando un canale di alert email non li specifica. |
...
| `alerts.enabled_channels` | email, Slack, webhook | Tipi di canale a livello organizzativo che le regole di alert possono inviare. |
...
- Controlla le modifiche di utente, chiave e organizzazione.
- Testa i requisiti di backup, conservazione e risposta agli incidenti per la tua distribuzione.

<Warning>
  Disattivare l'acquisizione dei trascritti cambia ciò che audit e investigazioni possono provare. Registra la decisione e i suoi limiti previsti.
</Warning>