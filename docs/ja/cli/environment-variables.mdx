...
| 変数 | 説明 |
|----------|-------------|
| `FAILPROOF_API_URL` | ダッシュボードの認証ダイアログが使用する API サーバーのベース URL を上書きする。デフォルトは `https://api.befailproof.ai`。ローカルの API サーバーを使用する場合は `http://localhost:8080`（または該当するアドレス）に設定する。 |
