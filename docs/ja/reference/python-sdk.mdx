---
title: "カスタムエージェント"
description: "カスタムエージェントのトレースをインストルメント化し、Failproof AI が実行を再構築して障害を検出できるようにします。"
icon: "python"
---
...
<Tabs>
  <Tab title="ダッシュボード">
    ...
    ![カスタム Python エージェントセッションが実行グラフと順序付きイベントトレースとして再構築されている様子。](/images/dashboard/session-detail.png)
  </Tab>
  <Tab title="CLI">
    ```bash
    failproofai config \
