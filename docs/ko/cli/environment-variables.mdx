...
| 변수 | 설명 |
|----------|-------------|
| `FAILPROOFAI_TELEMETRY_DISABLED=1` | 익명 사용 원격 측정 비활성화 |
...
| `FAILPROOFAI_AUTH_DIR` | `auth.json` 저장 위치 재정의 (기본값: `~/.failproofai`). 주로 격리된 테스트 환경에서 유용함. |
...
| `FAILPROOFAI_LLM_API_KEY` | LLM 기반 정책용 API 키 |
