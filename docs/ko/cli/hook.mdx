...
이 명령어는 `failproofai policies --install`이 Claude Code의 `settings.json`에 등록하는 명령어입니다. 일반적으로 직접 호출하지 않습니다.

stdin으로부터 JSON 페이로드를 읽고, 활성화된 모든 정책을 평가한 뒤, 결정 내용을 나타내는 종료 코드와 함께 종료됩니다:
