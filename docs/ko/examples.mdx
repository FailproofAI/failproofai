...
자주 쓰이는 시나리오에 바로 사용할 수 있는 예제입니다. 각 예제는 설치 방법과 예상 동작을 보여줍니다.

---

## Claude Code에 훅 설정하기

Failproof AI는 Claude Code의 [훅 시스템](https://docs.anthropic.com/en/docs/claude-code/hooks)을 통해 통합됩니다. `failproofai policies --install`을 실행하면 모든 도구 호출 시 동작하는 훅 명령어가 Claude Code의 `settings.json`에 등록됩니다.
...
[Agents SDK](https://docs.anthropic.com/en/docs/agents-sdk)를 사용해 개발 중이라면, 동일한 훅 시스템을 프로그래밍 방식으로 활용할 수 있습니다.
...
<Steps>
  ...
  <Step title="에이전트에 훅 구성">
    에이전트 프로세스를 생성할 때 훅 명령어를 전달하세요. 훅은 Claude Code와 동일한 방식으로, stdin/stdout JSON을 통해 동작합니다.
...
가장 일반적인 설정 — 에이전트가 되돌릴 수 없는 피해를 입히는 것을 방지합니다.
...
- `block-curl-pipe-sh` — 원격 스크립트를 셸로 파이프하는 동작 차단

---

## 시크릿 유출 방지

에이전트가 도구 출력에서 자격증명을 보거나 유출하는 것을 막습니다.

```bash
failproofai policies --install sanitize-api-keys sanitize-jwt sanitize-connection-strings sanitize-bearer-tokens
```

이 정책들은 `PostToolUse` 시점에 동작합니다 — 도구 실행 후, 에이전트가 결과를 보기 전에 출력을 정제합니다.

---

## 에이전트 대기 시 Slack 알림 받기

알림 훅을 사용해 유휴 알림을 Slack으로 전달합니다.
...
failproofai를 설치한 모든 팀원이 이 규칙을 자동으로 적용받습니다.

---

## 컨벤션 정책으로 조직 전체 품질 기준 구축하기

가장 효과적인 설정: 프로젝트에 맞게 조정된 정책을 `.failproofai/policies/` 디렉토리에 저장하고 저장소에 커밋합니다. 팀원 모두가 자동으로 적용받습니다 — 별도의 설치 명령이나 설정 변경 없이도요.
...
<Steps>
  ...
  <Step title="지속적으로 개선">
    팀이 새로운 문제 상황을 마주칠 때마다 정책을 추가하고 푸시하세요. `git pull` 때마다 모든 팀원이 업데이트를 받습니다. 이 정책들은 팀과 함께 성장하는 살아있는 품질 기준이 됩니다.
  </Step>
</Steps>
...
| 파일 | 내용 |
|------|------|
| `policies-basic.js` | 기본 정책 — 프로덕션 쓰기 차단, 강제 푸시 차단, 파이프 스크립트 차단 |
| `policies-notification.js` | 유휴 알림 및 세션 종료 시 Slack 알림 |
| `policies-advanced/index.js` | 전이적 임포트, 비동기 훅, PostToolUse 출력 정제, Stop 이벤트 처리 |