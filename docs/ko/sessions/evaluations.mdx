---
title: "온라인 평가"
description: "라이브 및 완료된 세션의 품질, 컴플라이언스, 비용, 레이턴시를 점수로 평가합니다."
icon: "gauge"
---
...
<Tabs>
  <Tab title="대시보드">
    ...
    4. 결과를 선택하여 일치하는 세션을 열거나 필터링된 뷰를 공유합니다. 레이턴시, 토큰, 비용 및 기타 수치 값은 **Observe → Metrics**를 사용하세요.

    ![평균 평가 점수와 시간에 따른 트렌드를 보여주는 품질 대시보드.](/images/dashboard/dashboard-quality.png)

    드릴다운에서 세션을 열어 점수별 추론 내용을 확인하세요:

    ![전체 트레이스 옆에 평가 점수와 추론을 표시하는 세션 상세 뷰.](/images/dashboard/session-detail.png)
  </Tab>
  <Tab title="CLI">
    ```bash
    ...
    자동화를 위해 `evals` 앞에 전역 `--json`을 추가하세요. 예: `fp --json evals --aggregate --env production`.
  </Tab>
</Tabs>
...
<Card title="평가자 빌드" icon="code-2" href="/ko/reference/evaluator-sdk">
