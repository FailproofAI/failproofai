...
{/* These four arrived with the hard-floor additions (build task T7). Two more
    were drafted — `block-mass-kill` and `block-no-verify` — and cut from the
    scope, so they are deliberately absent here. */}
