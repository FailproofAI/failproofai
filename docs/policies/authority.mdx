...
And because an instruct-mode semantic policy can never answer deny, pairing a *blocking* policy with instruct-mode checks alone leaves the conjunction one reachable outcome: whenever those checks are asked, the deny is cleared. Six of the built-in checks are instruct-only — `push-to-protected-branch`, `commit-on-protected-branch`, `read-outside-workspace`, `system-modification`, `env-secrets-dump` and `external-data-egress` — and the [table below](#semantic-policy-names) gives every check's mode. The question to ask is not "can the reviewer keep this block" but **"is there anything left that can deny"** — a policy may still be reviewable when a clear hands the decision to other deny-mode checks that are asked about the same call and merge at the most severe verdict. It must be hard when a clear leaves the concern enforced by nothing.
...
These are the built-in checks, and the values `reviewedBy` accepts on a machine where no installed pack declares Jev checks. Each is a check Jev answers about the tool call in front of it. **Mode** is what a check can answer: a `deny` check blocks on strong evidence, while an `instruct` check only ever warns, so it can never keep a deny standing. **User can override** says whether the human's own explicit request clears it.

A pack that declares [Jev checks](/policies/publish-a-pack#jev-checks-in-a-pack) replaces this list on every machine that installs it: the installed packs' checks are then the only ones Jev asks, and the only names `reviewedBy` accepts, so a policy naming a check below that no installed pack declares stays hard. `FailproofAI/jev-policies` declares these same sixteen, so with it the table still applies. A name two packs declare differently is honoured for neither, and so is one of these sixteen names declared by a pack not installed from a FailproofAI repository — a third-party pack cannot become the check that clears the core pack's policies. A pack whose every check is unusable leaves this list in force.
