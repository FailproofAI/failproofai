...
2. `reviewedBy` is a non-empty list, and every entry is a semantic check this machine can ask: one of the [built-in checks](#semantic-policy-names), or — once an installed pack declares Jev checks of its own — one of those packs' checks.
...
These are the built-in checks, and the values `reviewedBy` accepts on a machine where no installed pack declares Jev checks. Each is a check Jev answers about the tool call in front of it.

A pack that declares [Jev checks](/policies/publish-a-pack#jev-checks-in-a-pack) replaces this list on every machine that installs it: the installed packs' checks are then the only ones Jev asks, and the only names `reviewedBy` accepts, so a policy naming a check below that no installed pack declares stays hard. `FailproofAI/jev-policies` declares these same sixteen, so with it the table still applies. A name two packs declare differently is honoured for neither, and a pack whose every check is unusable leaves this list in force.
