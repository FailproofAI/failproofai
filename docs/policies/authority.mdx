...
Anything else is hard: a missing field, a misspelled value, an empty or malformed `reviewedBy`, or a name this version does not recognise. An unknown name makes the whole declaration hard rather than being skipped, because `reviewedBy` means "all of these must come back clear", and skipping a name would let Jev clear the policy on fewer checks than you asked for. Failproof AI logs a warning when it refuses a `reviewable` declaration.
...
| Cloud-managed policies | The policy's assignment in the active deployment | Hard |
...
`failproofai publish` copies both fields into the pack manifest, so a policy published as a pack keeps the authority its author gave it.
...
| `block-work-on-main` | reviewable | `commit-on-protected-branch` | The same concern: a commit written directly on a protected branch. |
...
| `block-rm-rf` | hard | | Catastrophic and irreversible. |
...
| `block-force-push` | hard | | Overwrites remote history. |
| `block-secrets-write` | hard | | Writes secret key files. |
| `block-kubectl` | hard | | Changes live cluster state. |
| `block-terraform` | hard | | Changes real infrastructure. |
| `block-aws-cli` | hard | | Changes cloud resources. |
| `block-gcloud` | hard | | Changes cloud resources. |
| `block-az-cli` | hard | | Changes cloud resources. |
| `block-helm` | hard | | Changes live cluster state. |
...
These built-in policies cover commands Jev is known to miss, so they are hard by design. They are off by default.

{/* These six arrive with the hard-floor additions (build task T7). If they do
    not ship in the same release, remove this table and the sentence above. */}

| Policy | Authority | Reviewed by | Why |
| --- | --- | --- | --- |
| `block-disk-destruction` | hard | | Covers `dd` to a device, `mkfs` and `wipefs`, which Jev misses. |
| `block-gh-destructive` | hard | | Covers `gh release delete` and `gh api -X DELETE`, which Jev misses. |
| `block-mass-kill` | hard | | Covers mass `pkill` and `killall`, which Jev misses. |
| `block-no-verify` | hard | | Covers `git commit` and `git push` with `--no-verify`, which Jev misses. |
| `block-indirect-exec` | hard | | Covers destructive binaries run through a variable, which Jev misses. |
| `block-chmod-777` | hard | | Covers world-writable permission changes. |
