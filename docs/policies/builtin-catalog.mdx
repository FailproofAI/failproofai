...
`block-failproofai-commands` is **always on**. It is listed above for
completeness, but it registers on every evaluation whether or not it appears in
your enabled set, and it cannot be disabled or paused — a guard against the
agent switching off enforcement that the agent can switch off is not a guard.
