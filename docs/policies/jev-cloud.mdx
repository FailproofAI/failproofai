...
`status` shows the provider as **FailproofAI Cloud**, the Cloud host the machine connected to, the mode, and the key source as **FailproofAI Cloud connection**, never the key. If the machine is not connected (or was disconnected), it says **off — this machine is not connected to FailproofAI Cloud**. `status --json` carries the same facts (`provider: "failproofai"`, `keySource: "cloud"`, `cloudConnected`). `test` sends one live request and reports its latency and the Jev version that answered.
