...
failproofai pack add acme/support-agent --only block-refunds
failproofai pack add acme/support-agent --all
```

`--category` and `--only` combine as a union. Re-adding at a newer version keeps whatever you chose rather than switching the rest back on.
...
## The builtin policies, as a pack

The policies that ship with failproofai are also published as a pack, and a copy travels inside the npm package:

```bash
failproofai pack add --bundled          # from this package, no network
failproofai pack add FailproofAI/policies   # from GitHub
```
