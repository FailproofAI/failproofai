...
Veja exatamente o que seus agentes de IA fizeram em produção: cada execução de agente, chamada de ferramenta, requisição de modelo, hook e intervenção humana. O Python SDK de Observabilidade do Failproof AI registra esse rastro dentro do código do seu agente para que você possa depurar, auditar e avaliar o que aconteceu. Use-o sempre que quiser que a Observabilidade do Failproof AI monitore seus agentes.

Por baixo dos panos, o SDK grava eventos estruturados em arquivos JSONL locais, e o daemon coletor os lê e os envia para a plataforma automaticamente. Você não precisa gerenciar esses arquivos diretamente.
...
O SDK é distribuído aos clientes como um wheel privado, e não por meio de um índice de pacotes público. O processo de integração explica como obtê-lo, instalá-lo e fixar sua versão — fale com seu contato do Failproof AI se precisar de acesso.

Após instalado, confirme que está disponível:
...
Na prática, você envolve o código existente do seu agente. Envolva uma chamada de modelo com `model_request` antes e `model_response` depois, para que os dois eventos abranjam a requisição real e a Observabilidade do Failproof AI possa correlacioná-los:
...
Veja como esses eventos aparecem no dashboard, com codificação de cores por tipo e filtros por ambiente, agente e sessão:

![O stream de Eventos ao vivo, com codificação de cores por tipo de evento e filtros por ambiente, agente e sessão](/agenteye/images/events-stream.png)
...
Chame uma vez antes de qualquer chamada `event.*`. Pode ser omitido com segurança; os valores padrão funcionam sem configuração adicional. Todos os argumentos são somente por nome (keyword-only); passe-os pelo nome conforme mostrado acima.

Quando `base_dir` é `None` (o padrão), o SDK lê `$AGENTEYE_HOME` se estiver definido,
caso contrário usa `~/.agenteye`. Isso corresponde à própria resolução do coletor,
portanto uma única variável de ambiente `AGENTEYE_HOME` configura o spool de eventos
...
> **Aviso:** Os valores de ambiente não devem conter uma vírgula `,` literal. Os filtros do dashboard usam multi-seleção separada por vírgulas na URL (`?environment=prod,staging`), portanto um ambiente chamado `prod,blue` seria dividido em dois valores. Eventos com ambientes contendo vírgulas são rejeitados no momento da ingestão.
...
Isso torna a redação sua escolha e sua responsabilidade. Se um prompt ou payload de ferramenta contiver PII ou segredos que você prefere não armazenar, remova ou mascare-os antes de passá-los ao método de evento.

---

## Referência de Eventos

A maioria dos eventos vem em pares início/fim que compartilham um ID de correlação: `tool_use` e `tool_result` compartilham um `tool_call_id`, `hook_triggered` e `hook_completed` compartilham um `hook_id`, e `human_wait` e `human_input` compartilham um `input_id`. Emita o evento de início, realize o trabalho e depois emita o evento de fim com o mesmo ID. A Observabilidade do Failproof AI correlaciona o par e calcula o `duration_ms` automaticamente, portanto você nunca precisa passar `duration_ms` manualmente.

![O grafo de execução estilo git de uma sessão ao lado de sua linha do tempo de eventos, reconstruído a partir dos eventos pareados, com o painel de detalhamento de ferramentas/modelos/hooks](/agenteye/images/session-detail.png)
...
As entradas de `messages` aceitam tanto uma string simples em `content` quanto `content` no estilo Anthropic como lista de blocos. Parâmetros de amostragem (`temperature`, `max_tokens`, etc.) podem ser passados como kwargs extras.
...
Emitido quando um hook dispara. Pareie com `hook_completed`; o SDK calcula `duration_ms` automaticamente.
...
Emitido quando o agente pausa a execução para aguardar uma entrada humana. Pareie com `human_input`; o SDK calcula `duration_ms` automaticamente (quanto tempo o humano levou para responder).
...
Emitido quando um humano fornece entrada e o agente retoma. Correlaciona-se com `human_wait` via `input_id`. O `duration_ms` é calculado automaticamente e não deve ser passado pelo chamador.
...
Cada arquivo é gravado atomicamente: o SDK grava em um arquivo temporário e depois o renomeia para o local definitivo, portanto o coletor nunca vê um arquivo parcialmente escrito. Um descarte final também é executado quando seu processo encerra, para que eventos armazenados no último intervalo não sejam perdidos. Se o coletor estiver offline, os eventos simplesmente se acumulam como arquivos em disco e são enviados assim que ele voltar.

---

## Próximos passos

- [Stream de eventos](/pt-br/agenteye/event-stream): acompanhe esses eventos chegando ao vivo, com codificação de cores e filtros por ambiente, agente e sessão.
