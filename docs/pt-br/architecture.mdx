...
1. **Hook handler** - Um subprocesso de CLI rápido que o Claude Code invoca em cada chamada de ferramenta do agente. Avalia políticas e retorna uma decisão.
2. **Agent Monitor (Dashboard)** - Uma aplicação web Next.js para monitorar sessões de agentes e gerenciar políticas.

Ambos os subsistemas compartilham arquivos de configuração em `~/.failproofai/` e no diretório `.failproofai/` do projeto, mas executam como processos separados e se comunicam apenas pelo sistema de arquivos.
...
Ao executar `failproofai policies --install`, entradas como esta são gravadas em `~/.claude/settings.json`:
...
O handler impõe um limite de 1 MB no stdin. Payloads que excedam esse limite são descartados e todas as políticas permitem implicitamente.
...
`allow(message)` permite que uma política envie contexto informativo de volta ao Claude mesmo quando a operação é permitida. O hook handler grava o seguinte JSON no **stdout** (não em um arquivo de configuração — esta é a resposta do handler ao Claude Code, assim como as respostas de deny e instruct acima):
...
- `policyParams` - por chave de política, o primeiro arquivo que a definir vence integralmente
- `customPoliciesPath` - o primeiro arquivo que o definir vence
- `llm` - o primeiro arquivo que o definir vence
...
1. Consulta o schema de `params` da política (se houver).
...
`src/hooks/builtin-policies.ts` define todas as 39 políticas nativas como objetos `BuiltinPolicyDefinition`:
...
A correspondência de padrões dentro das políticas usa tokens de comando parseados (argv), não correspondência de strings brutas. Isso evita bypass por injeção de operadores shell (ex.: um padrão para `sudo systemctl status *` não pode ser contornado adicionando `; rm -rf /` ao comando).
...
1. Lê `customPoliciesPath` da configuração; pula se ausente.
...
Em qualquer erro (arquivo não encontrado, erro de sintaxe, falha de importação), o erro é registrado em `~/.failproofai/hook.log` e o loader retorna um array vazio. As políticas builtin não são afetadas.

As políticas customizadas são avaliadas após todas as políticas builtin. Um `deny` de uma política customizada ainda interrompe as demais políticas customizadas (mas todos os builtins já terão sido executados até esse ponto).

---

## Log de atividade

Após cada evento de hook, o handler anexa uma linha JSONL em `~/.failproofai/hook-activity/current.jsonl`, que é rotacionado para `page-<timestamp>-<seq>.jsonl` ao atingir o tamanho de uma página:
...
app/
  ...
  projects/page.tsx           ← Componente servidor: lista todos os projetos Claude
  project/[name]/page.tsx     ← Componente servidor: lista sessões em um projeto
  project/[name]/session/
    [sessionId]/page.tsx      ← Componente servidor: renderiza o visualizador de sessão
  policies/page.tsx           ← Componente cliente: gerenciamento de políticas + log de atividade
  actions/
...
- A página de Políticas usa Server Actions para todas as mutações (ativar/desativar, atualização de parâmetros, instalação/remoção).
...
- Componentes cliente apenas onde a interatividade é necessária (alternância de políticas, busca de atividade, visualizador de log).
...
│   ├── policy-evaluator.ts       # Engine de execução de políticas
│   ├── policy-registry.ts        # Registro e consulta de políticas
...
├── examples/                     # Exemplos de arquivos de hook customizado
