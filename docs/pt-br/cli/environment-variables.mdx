...
| Variável | Descrição |
|----------|-----------|
| `FAILPROOF_API_URL` | Substitui a URL base do servidor de API usado pelo pelo diálogo de autenticação do dashboard. O padrão é `https://api.befailproof.ai`; defina como `http://localhost:8080` (ou outro endereço) ao executar um servidor de API local. |
| `FAILPROOFAI_AUTH_DIR` | Substitui o local onde o `auth.json` é armazenado (padrão: `~/.failproofai`). Útil principalmente para testes isolados. |

## Prompt de primeiro uso
