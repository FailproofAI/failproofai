...
<Steps>
  ...
  <Step title="Execute o Claude Code">
    ...
    As políticas agora rodam automaticamente em cada chamada de ferramenta. Tente pedir ao Claude para executar `sudo rm -rf /` — o comando será bloqueado.
  </Step>
</Steps>
...
Se você estiver desenvolvendo com o [Agents SDK](https://docs.anthropic.com/en/docs/agents-sdk), pode usar o mesmo sistema de hooks de forma programática.
...
<Steps>
  ...
  <Step title="Configure os hooks no seu agente">
    Passe os comandos de hook ao criar o processo do seu agente. Os hooks são acionados da mesma forma que no Claude Code — via JSON por stdin/stdout:
...
Impeça que agentes visualizem ou vazem credenciais na saída das ferramentas.
...
Impeça que agentes troquem de branch ou façam push em branches protegidas.
...
Faça commit de uma configuração no nível do projeto para que todos os desenvolvedores da sua equipe recebam as mesmas políticas.
...
Todo membro da equipe que tiver o failproofai instalado irá receber essas regras automaticamente.

---

## Construa um padrão de qualidade para toda a organização com políticas de convenção

A configuração de maior impacto: faça commit da pasta `.failproofai/policies/` no seu repositório com políticas adaptadas ao seu projeto. Todos os membros da equipe as recebem automaticamente — sem comandos de instalação, sem alterações de configuração.
...
<Steps>
  ...
  <Step title="Continue melhorando">
    À medida que sua equipe encontrar novos tipos de falha, adicione políticas e faça push. Todos recebem a atualização no próximo `git pull`. Essas políticas se tornam um padrão de qualidade vivo que cresce junto com a sua equipe.
  </Step>
</Steps>
