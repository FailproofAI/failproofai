...
## Por que mantemos a propriedade dos aliases
...
Para eliminar essa superfície de ataque, **nós registramos preventivamente todos os erros de digitação comuns e variantes de formatação** de `failproofai` no npm. Nenhum desses nomes pode ser registrado por terceiros. Cada um é um proxy simples que instala e delega para o pacote real `failproofai`.
...
**Erros de digitação `faliproof*`** - transposição de `a` e `i`:
...
O proxy é um script Node de duas linhas; não há nenhuma lógica extra, chamadas de rede ou coleta de dados além do que o próprio `failproofai` realiza.
