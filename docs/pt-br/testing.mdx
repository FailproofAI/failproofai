...
Os testes unitários ficam em `__tests__/` e usam o [Vitest](https://vitest.dev) com `jsdom`.

```text
__tests__/
  hooks/
    builtin-policies.test.ts      # Lógica de políticas para cada builtin
    hooks-config.test.ts          # Carregamento de config e mesclagem de escopos
...
__tests__/e2e/
  helpers/
    hook-runner.ts      # Inicializa o binário, envia o payload JSON, captura exit code + stdout + stderr
  ...
  hooks/
    ...
    config-scopes.e2e.test.ts      # Mesclagem de config entre escopos project/local/global
...
`createFixtureEnv()` registra a limpeza no `afterEach` automaticamente.
...
**`Payloads`** — factories de payload prontos para uso:
...
A execução completa de CI (`bun run lint && bunx tsc --noEmit && bun run test:run && bun run build`) deve passar antes de qualquer merge. O conjunto E2E é executado como um job de CI separado, em paralelo.
