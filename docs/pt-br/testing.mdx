...
__tests__/
  hooks/
    builtin-policies.test.ts      # Lógica de policies para cada builtin
    hooks-config.test.ts          # Carregamento de config e mesclagem de escopo
...
### Escrevendo um teste unitário de policy
...
__tests__/e2e/
  helpers/
    hook-runner.ts      # Inicia o binário, envia o payload JSON, captura exit code + stdout + stderr
    fixture-env.ts      # Diretórios temporários isolados por teste com arquivos de config
    payloads.ts         # Factories de payload fiéis ao Claude para cada tipo de evento
  hooks/
    builtin-policies.e2e.test.ts   # Cada policy builtin com subprocesso real
    custom-hooks.e2e.test.ts       # Carregamento e avaliação de hooks customizados
    config-scopes.e2e.test.ts      # Mesclagem de configs entre escopos projeto/local/global
    policy-params.e2e.test.ts      # Injeção de parâmetros para cada policy parametrizada
```
...
// env.home   - diretório home isolado; sem vazamento do ~/.failproofai real
...
**`Payloads`** - factories de payload prontas para uso:
...
A execução completa de CI (`bun run lint && bunx tsc --noEmit && bun run test:run && bun run build`) deve passar antes do merge. O conjunto de testes E2E é executado como um job de CI separado em paralelo.

Consulte [Contributing](../CONTRIBUTING.md) para o checklist completo pré-merge.