...
failproofai possui dois conjuntos de testes: **testes unitários** (rápidos, com mocks) e **testes de ponta a ponta** (invocações reais de subprocessos).
...
Os testes unitários ficam em `__tests__/` e utilizam o [Vitest](https://vitest.dev) com `jsdom`.
...
Os testes E2E invocam o binário real do `failproofai` como subprocesso, enviam um payload JSON para o stdin e verificam a saída no stdout e o código de saída. Isso testa o caminho completo de integração que o Claude Code utiliza.

### Configuração

Os testes E2E executam o binário diretamente do código-fonte do repositório. Antes da primeira execução, compile o bundle CJS que os arquivos de hook customizados utilizam ao importar de `'failproofai'`:
...
__tests__/e2e/
  helpers/
    hook-runner.ts      # Inicia o binário, envia o JSON do payload, captura código de saída + stdout + stderr
...
A execução completa de CI (`bun run lint && bunx tsc --noEmit && bun run test:run && bun run build`) precisa passar antes de qualquer merge. O conjunto de testes E2E é executado como um job de CI separado, em paralelo.
