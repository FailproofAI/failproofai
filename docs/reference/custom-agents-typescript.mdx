...
| **Vercel AI SDK** | Call-site APIs — see below. |
...
The AI SDK exports plain functions from an ES module, and an ES module namespace is immutable by specification — there is nowhere to patch. It uses the two extension points the SDK itself documents:
...
That is the complete integration: an agent span, model request/response with token counts, and every tool call.

If you would rather not pass it at each call site, wrap the model once instead — that sees model calls only, because tool calls happen above the model layer:
...
Using both is fine: the middleware notices an open tracer span and defers, so each call is recorded once.
