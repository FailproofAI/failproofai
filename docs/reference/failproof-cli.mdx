...
| `failproofai policy add <name>` | Enable one policy — a builtin, or `<pack-id>:<name>` from an installed pack |
| `failproofai policy remove <name>` | Disable one policy, same naming |
| `failproofai policies --uninstall` | Disable policies or remove harness hooks |
| `failproofai pack list` | List installed policy packs and every policy each one carries |
| `failproofai pack add <source>` | Install a policy pack from a GitHub release; no tag takes the newest and pins it |
| `failproofai pack add --bundled` | Install the builtin policies as a pack, from this package, with no network |
| `failproofai pack build <entry.mjs>` | Build the three release assets for a pack of your own |
| `failproofai pack remove <publisher/name>` | Deactivate an installed pack |
