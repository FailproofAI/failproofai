...
failproofai config                              # hooks, daemon, and Cloud in one pass
failproofai policies add FailproofAI/policies   # choose what enforces
...
Prefer the environment variable over `--token`: a command-line argument is readable from `ps` by every user on the box, and lands in shell history and CI logs.
...
| Variable | Use |
| --- | --- |
| `FAILPROOFAI_CLOUD_TOKEN` | The Cloud key, instead of `--token`. Prefer this: an argument is readable from `ps` and lands in shell history and CI logs |
