...
| `failproofai config --token <key>` | Set up and connect in one pass, asking nothing |
...
| `failproofai publish` | Ship your own policies as a pack; `--init` writes one to start from |
