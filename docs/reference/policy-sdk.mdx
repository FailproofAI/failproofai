...
| `PostToolUse` | After a tool returns. | Inspect results after the tool has run. A deny replaces the whole result on Codex and Copilot only; on other CLIs it appends a note and the agent still reads the real output. It never redacts selected fields. To prevent rather than report, use `PreToolUse`. |
