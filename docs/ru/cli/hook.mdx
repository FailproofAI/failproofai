...
Это команда, зарегистрированная в `settings.json` Claude Code командой `failproofai policies --install`. Обычно вы не вызываете это напрямую.

Читает JSON-полезную нагрузку из stdin, оценивает все включённые политики и завершается с кодом, указывающим на решение:
