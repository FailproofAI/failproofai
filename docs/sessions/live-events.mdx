...
Agents that do not run in one of the 12 supported [harnesses](/reference/harnesses) report the same event types through the [Python SDK](/reference/custom-agents), including the human-in-the-loop events (`human_wait`, `human_input`, `human_interrupt`) that gateway and production agents depend on.
