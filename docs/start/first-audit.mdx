...
Run the local one first. It needs nothing but the CLI. Findings stay local, while anonymous CLI telemetry is sent by default unless you set `FAILPROOFAI_TELEMETRY_DISABLED=1`.
...
<Tabs>
  <Tab title="Local">
    ...
    Analysis and findings stay on this machine. Anonymous CLI telemetry is sent by default unless disabled. After you opt into scheduling, each scan also sends machine metadata and may send a limited findings digest.
