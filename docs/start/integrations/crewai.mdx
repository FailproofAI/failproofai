...
The nesting is what makes the trace readable on a hierarchical crew: the manager sits under the crew, and each coworker it delegates to sits under the manager. CrewAI parents a delegated execution on the `delegate_work_to_coworker` tool event rather than on the manager directly, so the adapter follows that link — otherwise every agent comes out a sibling of every other and the delegation structure is lost.
