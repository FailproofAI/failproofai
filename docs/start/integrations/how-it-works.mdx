...
Keep `agent_id` low cardinality. It is the primary facet on every dashboard surface, and it is a `LowCardinality(String)` column — a per-run value in it degrades the column and fills the filter dropdown with one entry per run.

Adapters defend that column for you: a label that is a UUID or a long bare hex string becomes `main`, and a readable name carrying a per-run id is stripped back to the readable part, so `agent-3f9a1c2b-…` records as `agent`. The real id is kept on `fw_agent_id` / `fw_run_id`, where it is queryable without being a facet. A name that merely contains short segments like `agent-v2` or `step-3` is left alone.

The guard is for labels the *framework* chose. An `agent_id` you pass yourself — to `event.*`, or to `failproofai_sdk.agent(...)` — is taken exactly as given, because silently rewriting an explicit argument would be worse than the cardinality it prevents. Name your own spans accordingly.
...
<Warning>
  **The daemon ships your batches; it does not rewrite them.** `collector.redact` scrubs credential-shaped strings out of the events the daemon captures *itself* — CLI session transcripts and hook activity — and it is applied where those events are written, not where batches are shipped. A batch the SDK wrote goes to Cloud exactly as your process produced it, so a prompt or a tool argument that contains a key still contains it on arrival.

  That is deliberate: these are your own instrumentation calls, and rewriting their payloads in transit would mean the events you receive are not the events you emitted. It does mean the SDK is where you control payload contents — `capture_content=False` on the adapter, or simply not passing the secret to `input=` — and that setting `collector.redact` is not a substitute for either.
</Warning>
...
<Warning>
  **Call `instrument()` after you import your framework, not before.** Because auto-detection reads `sys.modules`, a bare `instrument()` above the `import langchain` line finds nothing, installs nothing and returns `()` — the process then runs with the SDK imported, the adapter apparently installed, and not one event emitted. It logs a warning saying exactly this, so check your logs if a run records nothing; naming the framework explicitly, `instrument("langchain")`, works from anywhere because it imports the adapter on request.
</Warning>
