...
<Note>
  **Name your nodes whatever you like.** A node's own run is identified by its shape — a non-leaf run carrying LangGraph's own step tag — not by its name, so `add_node("lookup_population", ToolNode([...]))` records the tool, and `add_node("ChatOpenAI", ...)` records the model call. Naming a node after the thing it runs used to make that thing's events disappear.
</Note>

<Note>
  **Streaming and token counts.** `.stream()` / `.astream()` do not emit per-token events — they fold into the closing `model_response` as `fw_chunks` and `fw_ttft_ms`. Token counts are a separate matter: OpenAI only sends usage on a streamed response when asked, so `model_response` carries no tokens unless you construct the model as `ChatOpenAI(..., stream_usage=True)`. The adapter records what the framework gives it; without that flag there is nothing to record.
</Note>
