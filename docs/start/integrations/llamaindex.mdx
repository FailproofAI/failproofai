...
In an `AgentWorkflow`, that name is also what the handoff is recorded under: the workflow is the parent span and each agent that takes a turn opens its own nested one, so `agent_id` tells you which agent did the work and `parent_id` tells you which workflow it belonged to. An agent handed control back later opens a second turn rather than reopening the first.
...
| `capture_messages` | Turn off for regulated data. Structure, timings, tokens, and outcomes are still recorded. |
...
<AccordionGroup>
  ...
  <Accordion title="Usage is populated but the token columns are empty">
    There is no standard usage field in LlamaIndex. The adapter tries several known shapes, and an integration naming its counters something new will not match. The raw dict always ships, so look at `usage` in the payload to see what your provider called them. A populated `usage` with empty token columns is deliberate: it beats a confident wrong number.
  </Accordion>
