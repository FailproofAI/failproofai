...
3. Copy the one-time secret, then read it into a shell on the target machine. `read -s` takes it at a prompt that does not echo, so it never appears in a command:
...
<Steps>
  ...
  <Step title="Choose what to enforce">
    ...
    The pack is fetched from its GitHub release, checksum-verified, and pinned to the exact tag it resolved. It carries 44 policies and switches on the 10 its manifest marks as safe to enable unattended. Use them to see local policy decisions and try enforcement before Failproof AI audits your sessions and writes policies for your agents.

    Read any pack before taking it with `failproofai policies show <owner>/<repo>`, and see [policy packs](/policies/packs) for taking only part of one.
