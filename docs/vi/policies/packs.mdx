---
title: "Policy packs"
description: "Cài đặt một bộ policies được công bố dưới dạng GitHub release, và quản lý những gì nó buộc thực thi."
icon: "package"
---
...
A pack this machine was told to enforce and cannot run **denies** the events its missing policies covered, rather than allowing them silently. See [Failure behavior](/vi/policies/failure-behavior). `failproofai pack list` names any pack in that state and exits non-zero.
...
Publishing your own pack: see [Publish a pack](/vi/policies/publish-a-pack).