---
title: Hook handler（内部）
description: "Claude Code 在每个工具事件时调用的子进程"
---
...
这是由 `failproofai policies --install` 注册到 Claude Code 的 `settings.json` 中的命令。通常情况下，您无需直接调用此命令。

该命令从 stdin 读取 JSON 载荷，评估所有已启用的策略，并通过退出码指示决策结果：

| 退出码 | 决策 | 效果 |
|--------|------|------|
| `0` | `allow` | 允许该操作 |
| `1` | `deny` | 阻止该操作 - Claude 将收到拒绝原因 |
| `2` | `instruct` | 将指导信息注入 Claude 的上下文 |
