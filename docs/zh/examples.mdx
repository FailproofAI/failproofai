...
适用于常见场景的即用示例，每个示例均展示安装方式及预期效果。

---

## 为 Claude Code 配置 hooks

Failproof AI 通过 Claude Code 的 [hooks 系统](https://docs.anthropic.com/en/docs/claude-code/hooks)与其集成。运行 `failproofai policies --install` 后，它会在 Claude Code 的 `settings.json` 中注册 hook 命令，在每次工具调用时触发。
...
<Steps>
  ...
  <Step title="在 agent 中配置 hooks">
    在创建 agent 进程时传入 hook 命令。这些 hooks 的触发方式与 Claude Code 中完全相同——通过 stdin/stdout JSON 传递：
...
这些策略在 `PostToolUse` 阶段触发——工具执行完毕后，在 agent 看到输出之前对其进行脱敏处理。
...
将项目级配置提交到仓库，使团队中的每位开发者都能应用相同的策略。
...
最具影响力的配置方式：将针对项目定制的策略提交至仓库的 `.failproofai/policies/` 目录，团队成员无需任何安装命令或配置变更，即可自动获取这些策略。
...
<Steps>
  ...
  <Step title="持续改进">
    每当团队遇到新的问题模式时，添加相应策略并推送。所有人在下次 `git pull` 时即可获得更新。这些策略将成为随团队一同成长的活性质量标准。
  </Step>
</Steps>
...
| 文件 | 内容说明 |
|------|---------------|
| `policies-basic.js` | 入门策略——拦截生产环境写入、强制推送、管道脚本执行 |
