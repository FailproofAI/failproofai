...
failproofai 包含两套测试套件：**单元测试**（速度快，使用 mock）和**端到端测试**（真实的子进程调用）。
...
E2E 测试会将真实的 `failproofai` 二进制文件作为子进程调用，通过 stdin 传入 JSON payload，并对 stdout 输出和退出码进行断言。这覆盖了 Claude Code 所使用的完整集成路径。

### 配置

E2E 测试直接从仓库源码运行二进制文件。在首次运行前，需要构建自定义 hook 文件从 `'failproofai'` 导入时所用的 CJS 包：
...
每当修改公开的 hook API（`src/hooks/custom-hooks-registry.ts`、`src/hooks/policy-helpers.ts` 或 `src/hooks/policy-types.ts`）后，需要重新构建 `dist/`。
...
// env.home   - 隔离的 home 目录；防止真实 ~/.failproofai 的数据泄入
...
- `testTimeout: 20_000` — 每个测试超时 20 秒（包含二进制启动 + hook 评估时间）

`forks` 池模式非常重要：基于线程的 worker 会共享 `globalThis`，可能干扰启动子进程的测试。基于进程的 forks 可以避免这一问题。
