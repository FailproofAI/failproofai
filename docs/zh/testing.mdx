...
failproofai 包含两套测试：**单元测试**（快速、使用模拟）和**端到端测试**（真实子进程调用）。
...
E2E 测试将真实的 `failproofai` 二进制文件作为子进程调用，通过 stdin 传入 JSON 负载，并对 stdout 输出和退出码进行断言。这覆盖了 Claude Code 所使用的完整集成路径。

### 配置准备

E2E 测试直接从仓库源码运行二进制文件。在第一次运行之前，需构建自定义 hook 文件从 `'failproofai'` 导入时所用的 CJS 包：
...
每当修改公开的 hook API（`src/hooks/custom-hooks-registry.ts`、`src/hooks/policy-helpers.ts` 或 `src/hooks/policy-types.ts`）后，请重新构建 `dist/`。
...
__tests__/e2e/
  helpers/
    ...
    fixture-env.ts      # 为每个测试提供独立的临时目录和配置文件
    payloads.ts         # 为各事件类型提供符合 Claude 规范的负载工厂函数
  hooks/
...
// env.home   - 隔离的 home 目录；不会受真实 ~/.failproofai 的影响
...
`createFixtureEnv()` 会自动注册 `afterEach` 清理钩子。
...
`forks` 池模式至关重要：基于线程的 worker 共享 `globalThis`，可能干扰启动子进程的测试；基于进程的 forks 可以避免这一问题。

---

## 持续集成

完整的 CI 流程（`bun run lint && bunx tsc --noEmit && bun run test:run && bun run build`）必须全部通过才能合并。E2E 测试套件作为独立的 CI 任务并行运行。
