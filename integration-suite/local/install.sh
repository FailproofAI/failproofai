...
# writes the cron line. Idempotent: re-running upgrades the image and rewrites
# the cron line rather than adding a second one.
#
# WHY AN INSTALLER AT ALL. The manual path is four commands, and three of them
# have a failure mode that is silent for a day: a work dir mounted at a
# different path inside than out, a secrets file with a stale CANARY_REF, and a
# cron line that runs but reports nowhere. Each is caught here, at install time,
# in front of a person — instead of at 06:17 tomorrow in front of nobody.
#
# The person running this is not expected to know anything about the canary.
# Whoever HAS the credentials fills in secrets.env and sends it; this script
# checks it is complete and refuses to schedule anything that cannot report.
#
# Flags:
...
# Marker, not the whole command: the cron line is rewritten on every install, so
# it has to be findable even after the command it contains changes.
CRON_MARKER="# failproofai-canary"

SECRETS_SRC="" ; RUN_NOW=0 ; DO_CRON=1 ; DRY=0 ; CRON_AT="17 6"
while [ $# -gt 0 ]; do
  case "$1" in
    --now)      RUN_NOW=1 ;;
    --no-cron)  DO_CRON=0 ;;
...
if [ -n "$SECRETS_SRC" ]; then
  ...
  die "Fill in $WORK/secrets.env, then re-run this installer.
       ...
       COPILOT_GITHUB_TOKEN and CANARY_SLACK_WEBHOOK."
fi
...
if [ "$DRY" = 0 ]; then
  ...
  missing=""
  for v in CANARY_REF CANARY_LLM_API_KEY COPILOT_GITHUB_TOKEN CANARY_SLACK_WEBHOOK; do
    [ -n "$(getvar "$v")" ] || missing="$missing $v"
  done
  [ -z "$missing" ] || die "these are empty in $WORK/secrets.env:$missing

       CANARY_SLACK_WEBHOOK is required on purpose — a canary that runs and
       reports nowhere is worse than no canary, because it looks like coverage."

  CANARY_REF="$(getvar CANARY_REF)"
  # The shipped template still says origin/failproofaid, from before that branch
  # merged. Left alone it points the box at a ref that no longer moves, so the
  # canary would test a frozen tree forever and never say so.
  case "$CANARY_REF" in
    origin/failproofaid)
      die "CANARY_REF is still origin/failproofaid — that branch has merged.
       Set it to:  CANARY_REF=origin/main" ;;
  esac
  ok "env file complete — testing $CANARY_REF"
else
...
CMD="docker run --rm -v /var/run/docker.sock:/var/run/docker.sock -v \"$WORK:$WORK\" --env-file \"$WORK/secrets.env\" $IMAGE"

if [ "$DO_CRON" = 1 ]; then
  step "Scheduling the daily run"
  LINE="$CRON_AT * * * $CMD >/dev/null 2>&1 $CRON_MARKER"
  if [ "$DRY" = 1 ]; then
    say "would install cron line:"; say "$LINE"
  else
    # Drop any line we installed before, then add the current one — so a
    # re-install upgrades the schedule instead of stacking a second job.
    { crontab -l 2>/dev/null | grep -vF "$CRON_MARKER" || true; echo "$LINE"; } | crontab -
    did "cron installed — daily at $(echo "$CRON_AT" | awk '{printf "%02d:%02d", $2, $1}')"
    say "cron output goes to /dev/null on purpose: a leg that dies before it can"
...
say "all 12 CLIs. After that only a CLI whose version changed is re-probed, so"
say "normal days are short. Either way it posts a verdict to Slack when it ends."

if [ "$RUN_NOW" = 1 ]; then
  step "Running one canary now"
  run docker run --rm \
