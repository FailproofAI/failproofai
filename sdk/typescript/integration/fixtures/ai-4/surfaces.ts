// Vercel AI SDK 4.x: every commonly used surface. Run as `node surfaces.{mjs,cjs} <case>`.
//
...
// customer actually uses — embeddings, structured output, every tool feature,
// every way of consuming a stream, reasoning models and concurrency — against
// the same scripted, offline mock. (ai 4 has no agent class.)
//
...
// --- version-specific: ai 4 (LanguageModelV1, maxSteps, no agent class, no tool approval)
...
const usageOf = (step: Step) => ({ promptTokens: step.usage[0], completionTokens: step.usage[1] });
const finishOf = (step: Step) => (step.calls?.length ? "tool-calls" : "stop");
...
function model(steps: Step[], options: ModelOptions = {}) {
  ...
  const toolCalls = (step: Step) =>
    (step.calls ?? []).map((call) => ({ toolCallType: "function" as const, toolCallId: call.id, toolName: call.name, args: JSON.stringify(call.input) }));
  return new MockLanguageModelV1({
    ...
    doGenerate: async () => {
      ...
      return {
        ...
        ...(step.calls?.length ? { toolCalls: toolCalls(step) } : {}),
    ...
    doStream: async ({ abortSignal }) => {
      ...
      if (step.reasoning) for (const textDelta of halves(step.reasoning)) chunks.push({ type: "reasoning", textDelta });
      if (step.text) for (const textDelta of halves(step.text)) chunks.push({ type: "text-delta", textDelta });
      for (const call of toolCalls(step)) chunks.push({ type: "tool-call", ...call });
      chunks.push({ type: "finish", finishReason: finishOf(step), usage: usageOf(step) });
      return { stream: abortable(simulateReadableStream({ chunks, chunkDelayInMs: options.chunkDelayMs ?? 0 }), abortSignal, options.breakAfterParts), rawCall };
    },
  });
}

function embedder() {
  return new MockEmbeddingModelV1<string>({
    provider: "mock-provider",
    modelId: "mock-embedder",
    maxEmbeddingsPerCall: 2,
    doEmbed: async ({ values }) => ({ embeddings: values.map((_, i) => [i, 0.5]), usage: { tokens: values.length * 3 } }),
  });
}
...
const clientTool = () => tool({ description: "Ask the user", parameters: citySchema });
/** ai 4 has no tool approval (`needsApproval` arrived in 6). */
const HAS_APPROVAL = false;

const steps = (n: number) => ({ maxSteps: n });
/** ai 4 has no agent class (`Experimental_Agent` arrived in 5). */
const HAS_AGENT = false;

type Settings = Parameters<typeof generateText>[0];
type Telemetry = NonNullable<Settings["experimental_telemetry"]>;

function makeAgent(_options: { model: Settings["model"]; telemetry?: Telemetry; id?: string; tools?: Settings["tools"] }): {
  generate(prompt: string): Promise<string>;
  stream(prompt: string): Promise<string>;
} {
  throw new Error("ai 4 has no agent class");
}

/** `generateText` / `streamText` with `experimental_output: Output.object(...)`. */
const HAS_OUTPUT = true;
async function textWithOutput(m: Settings["model"], tel: Telemetry): Promise<unknown> {
  const result = await generateText({ model: m, prompt: "Where?", experimental_output: Output.object({ schema: citySchema }), experimental_telemetry: tel });
  return result.experimental_output;
}
async function streamWithOutput(m: Settings["model"], tel: Telemetry): Promise<unknown[]> {
  const result = streamText({ model: m, prompt: "Where?", experimental_output: Output.object({ schema: citySchema }), experimental_telemetry: tel });
  const partials: unknown[] = [];
  for await (const partial of result.experimental_partialOutputStream) partials.push(partial);
  return partials;
}

/** What a Next.js route handler returns (ai 4: the data stream protocol). */
const toResponse = (result: { toDataStreamResponse(): Response }): Response => result.toDataStreamResponse();

const repairOption = (fixed: Record<string, unknown>) => ({
  experimental_repairToolCall: async <T extends { args: string }>({ toolCall }: { toolCall: T }) => ({ ...toolCall, args: JSON.stringify(fixed) }),
});
const prepareStepOption = (second: Settings["model"]) => ({
  experimental_prepareStep: async ({ stepNumber }: { stepNumber: number }) => (stepNumber === 1 ? { model: second } : undefined),
});

async function approveAndResume(_m: Settings["model"], _tel: Telemetry): Promise<{ pending: number; text: string }> {
  throw new Error("ai 4 has no tool approval");
}
