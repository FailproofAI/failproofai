// Vercel AI SDK 5.x: every commonly used surface. Run as `node surfaces.{mjs,cjs} <case>`.
//
...
// --- version-specific: ai 5 (LanguageModelV2, Experimental_Agent, no tool approval)
...
type Content = Awaited<ReturnType<MockLanguageModelV2["doGenerate"]>>["content"][number];
...
const usageOf = (step: Step) => ({
  inputTokens: step.usage[0],
  outputTokens: step.usage[1],
  totalTokens: step.usage[0] + step.usage[1],
  ...(step.reasoningTokens === undefined ? {} : { reasoningTokens: step.reasoningTokens }),
});
...
function embedder() {
  return new MockEmbeddingModelV2<string>({
    provider: "mock-provider",
...
/** ai 5 has no tool approval (`needsApproval` arrived in 6). */
...
/** ai 5's agent class has no `id`; the option is accepted and ignored here. */
...
async function approveAndResume(_m: Settings["model"], _tel: Telemetry): Promise<{ pending: number; text: string }> {
  throw new Error("ai 5 has no tool approval");
}
