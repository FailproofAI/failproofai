...
// a workflow run is created with `createRunAsync()`, and an MCPClient lists its
// tools with `getTools()` / `getToolsets()`.
//
...
function workflowCase(name: string): { createRunAsync(): Promise<any> } {
...
async function main(scenario: string): Promise<void> {
  ...
  switch (scenario) {
    ...
    case "mastra-instance": {
      ...
      const run = await mastra.getWorkflow("weatherFlow").createRunAsync();
    ...
    case "wf-agent-step": {
      ...
      const run = await workflowCase(name).createRunAsync();
    ...
    case "wf-stream": {
      ...
      const run = await workflowCase("agent-step").createRunAsync();
    ...
    case "wf-suspend": {
      ...
      const run = await (mastra.getWorkflow("approval") as unknown as ReturnType<typeof workflowCase>).createRunAsync();
    ...
    case "mcp-toolsets": {
      ...
      try {
        if (scenario === "mcp") {
          ...
          const out = await agent.generate(question, { toolsets: await mcp.getToolsets() });
          report({ answer: out.text });
        }
      } finally {
    ...
    case "agent-in-tool": {
      ...
      const ask = createTool({
        ...
        inputSchema: z.object({ city: z.string() }),
        execute: async ({ context }: { context: { city: string } }) => ({ answer: (await helper.generate(`What is the weather in ${context.city}?`)).text }),
      });
