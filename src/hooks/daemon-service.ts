...
export type ConfigChangeReload =
  /** No service to reload — nothing was configured, or the platform has none. */
  | { reloaded: false; reason: "no-service" }
  /** A service exists but was not running; the next start reads the new file. */
  | { reloaded: false; reason: "not-running" }
  /** Restarted, and it answered a hook afterwards. */
  | { reloaded: true }
  /** Could not restart. `command` is what the operator must run by hand. */
  | { reloaded: false; reason: "no-elevation" | "restart-failed" | "did-not-return"; command: string };

/**
 * Restart failproofaid so it picks up a configuration or credential change.
 *
 * THE DAEMON READS ITS CREDENTIAL EXACTLY ONCE, at collector start —
 * `main.rs` starts the collector manager once and never tears it down, and the
 * uploader caches its bearer key at construction. Config is re-read on a 5s
 * tick; the CREDENTIAL is not. So rotating a key left a daemon posting the old
 * one forever, and the failure is invisible from every angle a person can see:
 * `--connect` verifies the NEW key itself and reports success, the service is
 * healthy, the file on disk is correct — and every batch 401s and parks. The
 * only symptom is data that never arrives. Verified live: a key revoked at
 * 13:05:37 and replaced 37 seconds later still produced 401s twenty minutes
 * on, with 26 batches parked and a CLI that said "connected".
 *
 * This was already KNOWN — `--disconnect` prints a note telling the user to
 * restart by hand. Printing the remedy is strictly worse than applying it: it
 * relies on someone reading past a success message to find out the success is
 * conditional.
 *
 * Deliberately NOT called on the hook path, and worth being explicit about the
 * cost: on a `daemonConfigured` machine every tool call routes through this
 * service and FAILS CLOSED, so a restart is a short window in which calls are
 * denied. That is acceptable here because these are operator-initiated commands
 * where a person is present and expecting change, and unacceptable anywhere
 * else. `probeDaemon()` afterwards is what keeps the window bounded and
 * verified rather than assumed — the same probe setup uses, so a daemon that
 * comes back unable to evaluate is reported instead of being called a success.
 */
export async function reloadDaemonAfterConfigChange(
  // Injected the way `connectionStatusLines` takes `daemonStatus`: these are
  // module-internal calls, so a test cannot intercept them by spying on the
  // exports — ESM binds them at module scope. Passing them makes the decision
  // table testable without mocking the filesystem and `sudo` underneath it.
  deps: {
    status?: () => DaemonServiceStatus;
    elevate?: () => boolean;
    probe?: () => Promise<DaemonProbe>;
  } = {},
): Promise<ConfigChangeReload> {
  const readStatus = deps.status ?? daemonServiceStatus;
  const canRestart = deps.elevate ?? canElevate;
  const verify = deps.probe ?? probeDaemon;

  if (!isDaemonSupportedPlatform()) return { reloaded: false, reason: "no-service" };

  const status = readStatus();
  if (status === "not-installed") return { reloaded: false, reason: "no-service" };
  // A stopped or condition-skipped service will read the new file whenever it
  // next starts. Restarting one that is deliberately down is not this
  // function's business, and on `condition-failed` it cannot start anyway.
  if (status !== "running") return { reloaded: false, reason: "not-running" };

  const command = daemonRestartCommand() ?? "restart failproofaid";
  if (!canRestart()) return { reloaded: false, reason: "no-elevation", command };
  ...
  try {
    ...
    hookLogWarn(
      `could not restart failproofaid after a config change: ${
        err instanceof Error ? err.message : String(err)
      }`,
    );
    return { reloaded: false, reason: "restart-failed", command };
  }

  // Not "systemctl said ok": a `Type=simple` unit is reported active the
  // instant systemd forks it, and the question here is whether the thing can
  // EVALUATE again. `probeDaemon` retries until the socket binds.
  const probe = await verify();
  return probe.ok ? { reloaded: true } : { reloaded: false, reason: "did-not-return", command };
}
