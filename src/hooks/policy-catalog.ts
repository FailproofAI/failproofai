...
export const POLICY_CATALOG: PolicyCatalogEntry[] = [
  ...
  // under its category's section.
  {
    name: "block-disk-destruction",
    displayTitle: "Tried to format, wipe or overwrite a disk device",
    impact: "`dd of=/dev/sda`, `mkfs` and `wipefs` destroy a whole disk in one call.",
    description: "Block formatting, wiping or writing raw data to disk devices (dd of=/dev/…, mkfs, wipefs, blkdiscard, diskutil erase, redirects to /dev/sd*)",
    match: { events: ["PreToolUse"], toolNames: ["Bash"] },
    defaultEnabled: false,
    category: "Dangerous Commands",
    authority: "hard",
  ...
  {
    name: "block-gh-destructive",
    displayTitle: "Tried to delete GitHub resources with the gh CLI",
    impact: "`gh release delete`, `gh repo delete` and `gh api -X DELETE` remove things no revert brings back.",
    description: "Block gh CLI commands that delete GitHub resources (release/repo/issue/run/secret/… delete, gh api -X DELETE, GraphQL delete mutations)",
    match: { events: ["PreToolUse"], toolNames: ["Bash"] },
    defaultEnabled: false,
    category: "Infra Commands",
    authority: "hard",
  },
  {
    ...
    description: "Block killing processes en masse (killall/pkill of interpreters, shells, or a pattern written with regex syntax; kill -1; and any kill fed by a ps/pgrep/top//proc listing the lister itself did not narrow, whatever the pipeline in between does)",
    match: { events: ["PreToolUse"], toolNames: ["Bash"] },
    defaultEnabled: false,
    category: "Dangerous Commands",
    authority: "hard",
  },
  {
    ...
    description: "Block bypassing git hooks (git commit/push --no-verify, git commit -n, core.hooksPath overrides, HUSKY=0) and any commit or push whose alias or git config the command does not state in full",
    match: { events: ["PreToolUse"], toolNames: ["Bash"] },
    defaultEnabled: false,
    category: "Git",
    authority: "hard",
  },
  {
    ...
    displayTitle: "Tried to run rm/dd/mkfs through a variable or other indirection",
    impact: "`R=/bin/rm; $R -rf …` hides a destructive command from every check that reads commands literally.",
    description: "Block running rm, dd, mkfs and similar through a variable, command substitution, glob or brace expansion",
    match: { events: ["PreToolUse"], toolNames: ["Bash"] },
    defaultEnabled: false,
    category: "Dangerous Commands",
    authority: "hard",
  },
  {
    ...
    displayTitle: "Tried to make files world-writable (chmod 777)",
    impact: "A world-writable file can be rewritten by any user or process on the machine.",
    description: "Block making files world-writable (chmod 777 / 666 / a+rwx / o+w, mkdir -m 777, icacls Everyone:F)",
    match: { events: ["PreToolUse"], toolNames: ["Bash"] },
    defaultEnabled: false,
    category: "Dangerous Commands",
    authority: "hard",
